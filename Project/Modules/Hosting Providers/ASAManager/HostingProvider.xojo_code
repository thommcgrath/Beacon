...
Implements Beacon.HostingProvider
	...
	#tag Method, Flags = &h0
		Sub UploadFile(Project As Beacon.Project, Profile As Beacon.ServerProfile, Transfer As Beacon.IntegrationTransfer)
		  ...
		  Var Response As ASAManager.APIResponse = Self.RunRequest(New ASAManager.APIRequest("PUT", ASAManager.APIRoot + "/containers/" + ContainerId.ToString(Locale.Raw, "0") + "/config/" + FileType, Token, "application/json", Body.ToString(False)))
		  If Not Response.Success Then
		    Transfer.Success = False
		    Transfer.ErrorMessage = Response.Error.Message
		    Return
