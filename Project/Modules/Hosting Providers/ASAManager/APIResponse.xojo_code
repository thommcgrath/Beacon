...
Private Class APIResponse
	...
	#tag Method, Flags = &h0
		Shared Function FromSocket(Socket As SimpleHTTP.SynchronousHTTPSocket) As ASAManager.APIResponse
		  Var Response As New ASAManager.APIResponse
		  ...
		  If SocketErr Is Nil Then
		    Var Err As New ASAManager.APIException
		    Err.ErrorNumber = Response.mHTTPStatus
		    Select Case Response.mHTTPStatus
		    Case 401
		      Err.Message = "ASAManager rejected the API token."
		    Case 404
		      Err.Message = "ASAManager endpoint not found."
		    Case 429
		      Err.Message = "ASAManager rate limit exceeded."
		    Case 500
		      Err.Message= "ASAManager internal server error."
		    Else
		  ...
		  App.Log("ASAManager API Error #" + Response.mError.ErrorNumber.ToString(Locale.Raw, "0") + ": " + Response.mError.Message)
