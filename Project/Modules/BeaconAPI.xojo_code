...
Protected Module BeaconAPI
		...
		Protected Function GetProviderToken(TokenId As String, Project As Beacon.Project, UseCache As Boolean = False) As BeaconAPI.ProviderToken
		  ...
		  Try
		    ...
		    If Token.TokenId <> TokenId Then
		      ...
		      App.Log("Provider token " + TokenId + " has changed to " + Token.TokenId + ".")
		      Beacon.Cache.Store(TokenId, Token, 10)
		      NotificationKit.Post(BeaconAPI.Notification_TokenIdUpdated, New Dictionary("oldTokenId": TokenId, "newTokenId": Token.TokenId))
		    End If
		    Return Token
		  Catch Err As RuntimeException
	...
	#tag Constant, Name = Notification_TokenIdUpdated, Type = String, Dynamic = False, Default = \"com.thezaz.beacon.token_id_updated", Scope = Protected
	#tag EndConstant
