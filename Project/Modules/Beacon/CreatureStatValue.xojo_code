...
Protected Class CreatureStatValue
	...
	#tag Method, Flags = &h0
		Sub Constructor(Stat As Beacon.Stat, BaseValue As Double, WildMultiplier As Double, TamedMultiplier As Double, AddMultiplier As Double, AffinityMultiplier As Double)
		  Self.mStat = Stat
	...
	#tag Method, Flags = &h0
		Shared Function FromSaveData(Dict As Dictionary) As Beacon.CreatureStatValue
		  ...
		  Var Stat As Beacon.Stat = Beacon.Stats.WithIndex(Dict.Value("stat_index").IntegerValue)
		  If Stat Is Nil Then
		    Return Nil
		  End If
		  
		  Return New Beacon.CreatureStatValue(Stat, Dict.Value("base_value").DoubleValue, Dict.Value("per_level_wild_multiplier").DoubleValue, Dict.Value("per_level_tamed_multiplier").DoubleValue, Dict.Value("add_multiplier").DoubleValue, Dict.Value("affinity_multiplier").DoubleValue)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Operator_Compare(Other As Beacon.CreatureStatValue) As Integer
