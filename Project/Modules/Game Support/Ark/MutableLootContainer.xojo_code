...
Implements Ark.MutableBlueprint
	...
	#tag Method, Flags = &h0
		Sub Requirements(Assigns Dict As JSONItem)
		  If Dict Is Nil Then
		  ...
		  If Beacon.Hash(Dict.ToString(False)) <> Beacon.Hash(Self.mRequirements.ToString(False)) Then
		    Self.mRequirements = New JSONItem(Dict.ToString(False))
		    Self.Modified = True
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Sub Unpack(Dict As JSONItem)
		  ...
		  For Each PackedSet As JSONEntry In Sets.Iterator
		    Var Set As Ark.LootItemSet = Ark.LootItemSet.FromSaveData(JSONItem(PackedSet.Value))
		    If (Set Is Nil) = False Then
