		...
		Shared Function FromSaveData(Dict As Dictionary, Options As Integer = 0) As Ark.LootItemSetEntry
		  ...
		  Try
		    If Dict.HasKey("useCustomQualities") Then
		      ...
		      Entry.MinQuality = Ark.Qualities.ForKey(Dict.Value("minQuality").StringValue)
		      Entry.MaxQuality = Ark.Qualities.ForKey(Dict.Value("maxQuality").StringValue)
	...
	#tag Method, Flags = &h0
		Shared Function ImportFromConfig(Dict As Dictionary, Multipliers As Beacon.Range, Difficulty As Double, ContentPacks As Beacon.StringList) As Ark.LootItemSetEntry
		  ...
		  If CustomData.HasKey("useCustomQualities") And CustomData.Value("useCustomQualities").BooleanValue Then
		    ...
		    Entry.MinQuality = Ark.Qualities.ForBaseValue(Entry.MinQualityOverride.DoubleValue)
		    Entry.MaxQuality = Ark.Qualities.ForBaseValue(Entry.MaxQualityOverride.DoubleValue)
		  Else
	...
	#tag Method, Flags = &h0
		Function SaveData(ForPreferences As Boolean = False) As Dictionary
		  ...
		  If Self.UseCustomQualities And (Self.MinQualityOverride Is Nil) = False And (Self.MaxQualityOverride Is Nil) = False Then
		    ...
		    Keys.Value("minQuality") = Ark.Qualities.ForBaseValue(Self.MinQualityOverride.DoubleValue).Key
		    Keys.Value("maxQuality") = Ark.Qualities.ForBaseValue(Self.MaxQualityOverride.DoubleValue).Key
		  Else
