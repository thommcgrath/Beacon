...
Protected Class LootSimulatorSelection
	#tag Method, Flags = &h0
		Shared Sub ComputeSimulatorFigures(Pool() As Ark.Weighted, WeightScale As Integer, ByRef WeightSum As Double, ByRef Weights() As Double, ByRef WeightLookup As Dictionary)
		  Weights.ResizeTo(-1)
		  WeightLookup = New Dictionary
		  WeightSum = 0
		  
		  For Each Item As Ark.Weighted In Pool
