...
Protected Module DedicatedServer
	#tag Method, Flags = &h1
		Protected Function Configure(Project As ArkSA.Project, Profile As ArkSA.ServerProfile, ArkRoot As FolderItem, HostDir As FolderItem) As Boolean
		  Var Organizer As ArkSA.ConfigOrganizer = Project.CreateConfigOrganizer(App.IdentityManager.CurrentIdentity, Profile)
		  ...
		  If ConfigFolder.Child(ArkSA.ConfigFileGameUserSettings).Exists Then
		    Var Stream As TextInputStream = TextInputStream.Open(ConfigFolder.Child(ArkSA.ConfigFileGameUserSettings))
		    GameUserSettingsIniOriginal = Stream.ReadAll(Nil).GuessEncoding
		    Stream.Close
		  End If
		  If ConfigFolder.Child(ArkSA.ConfigFileGame).Exists Then
		    Var Stream As TextInputStream = TextInputStream.Open(ConfigFolder.Child(ArkSA.ConfigFileGame))
		  ...
		  Var GameIniRewritten As String = ArkSA.Rewriter.Rewrite(ArkSA.Rewriter.Sources.Deploy, GameIniOriginal, ArkSA.HeaderShooterGame, ArkSA.ConfigFileGame, Organizer, Project.ProjectId, Project.LegacyTrustKey, Format, ArkSA.Project.UWPCompatibilityModes.Never, False, RewriteError)
		  If (RewriteError Is Nil) = False Then
		    App.Log(RewriteError, CurrentMethodName, "Building Game.ini")
		    Return False
		  End If
		  
		  Var GameUserSettingsIniRewritten As String = ArkSA.Rewriter.Rewrite(ArkSA.Rewriter.Sources.Deploy, GameUserSettingsIniOriginal, ArkSA.HeaderServerSettings, ArkSA.ConfigFileGameUserSettings, Organizer, Project.ProjectId, Project.LegacyTrustKey, Format, ArkSA.Project.UWPCompatibilityModes.Never, False, RewriteError)
		  If (RewriteError Is Nil) = False Then
		    App.Log(RewriteError, CurrentMethodName, "Building GameUserSettings.ini")
		    Return False
		  End If
		  
		  If ConfigFolder.Child(ArkSA.ConfigFileGameUserSettings).Write(GameUserSettingsIniRewritten) = False Then
		    App.Log("Could not write GameUserSettings.ini to platform config directory")
		    Return False
		  End If
		  If ConfigFolder.Child(ArkSA.ConfigFileGame).Write(GameIniRewritten) = False Then
