...
Implements Beacon.Countable,Iterable,ArkSA.Weighted,Beacon.Validateable
	...
	#tag Method, Flags = &h0
		Sub Constructor(Source As ArkSA.LootItemSet)
		  Self.Constructor()
	...
	#tag Method, Flags = &h1
		Protected Sub CopyFrom(Source As ArkSA.LootItemSet)
	...
	#tag Method, Flags = &h0
		Function FindEngram(EngramId As String) As ArkSA.BlueprintReference
		  For Each Entry As ArkSA.LootItemSetEntry In Self.mEntries
		    Var Reference As ArkSA.BlueprintReference = Entry.FindEngram(EngramId)
	...
	#tag Method, Flags = &h0
		Shared Function FromTemplate(Template As ArkSA.LootTemplate, ForLootContainer As ArkSA.LootContainer, Mask As UInt64, ContentPacks As Beacon.StringList) As ArkSA.LootItemSet
		  Var Set As New ArkSA.MutableLootItemSet
		  ...
		  Var Qualities() As ArkSA.Quality = ArkSA.Qualities.All
		  
		  For Each TemplateEntry As ArkSA.LootTemplateEntry In Template
	...
	#tag Method, Flags = &h0
		Shared Function FromTemplate(Template As ArkSA.LootTemplate, ForLootOverride As ArkSA.LootDropOverride, Mask As UInt64, ContentPacks As Beacon.StringList) As ArkSA.LootItemSet
		  Var LootDrop As ArkSA.LootContainer = ForLootOverride.LootDrop(ContentPacks)
	...
	#tag Method, Flags = &h0
		Function ImmutableClone() As ArkSA.LootItemSet
	...
	#tag Method, Flags = &h0
		Shared Function ImportFromConfig(Dict As Dictionary, Multipliers As Beacon.Range, Difficulty As Double, ContentPacks As Beacon.StringList) As ArkSA.LootItemSet
		  Var Set As New ArkSA.MutableLootItemSet
		  If Dict.HasKey("NumItemsPower") Then
		    Set.NumItemsPower = Dict.Value("NumItemsPower")
		  End If
		  ...
		  For Each Child As Variant In Children
		    Try
		      ...
		      Var Entry As ArkSA.LootItemSetEntry = ArkSA.LootItemSetEntry.ImportFromConfig(Dictionary(Child), Multipliers, Difficulty, ContentPacks)
		      If Entry <> Nil Then
		        Set.Add(Entry)
		      End If
		    Catch Err As RuntimeException
	...
	#tag Method, Flags = &h0
		Function IndexOf(Entry As ArkSA.LootItemSetEntry) As Integer
	...
	#tag Method, Flags = &h0
		Function MutableClone() As ArkSA.MutableLootItemSet
		  Return New ArkSA.MutableLootItemSet(Self)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Operator_Compare(Other As ArkSA.LootItemSet) As Integer
	...
	#tag Method, Flags = &h0
		Function Operator_Subscript(Index As Integer) As ArkSA.LootItemSetEntry
	...
	#tag Method, Flags = &h0
		Function Rebuild(ForContainer As ArkSA.LootContainer, Mask As UInt64, ContentPacks As Beacon.StringList) As ArkSA.LootItemSet
		  If Self.mTemplateUUID.IsEmpty Then
		    Return Nil
		  End If
		  
		  Var Template As ArkSA.LootTemplate = ArkSA.DataSource.Pool.Get(False).GetLootTemplateByUUID(Self.TemplateUUID)
		  If Template Is Nil Then
		    Return Nil
		  End If
		  
		  Var Clone As ArkSA.LootItemSet = Self.FromTemplate(Template, ForContainer, Mask, ContentPacks)
		  If Self.Hash(True) = Clone.Hash(True) Then
		    // No change
		    Return Nil
		  End If
		  
		  Var Mutable As New ArkSA.MutableLootItemSet(Clone)
		  ...
		  Return New ArkSA.LootItemSet(Mutable)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Simulate() As ArkSA.LootItemSetEntry()
		  ...
		  Var Pool() As ArkSA.LootItemSetEntry
		  Pool.ResizeTo(Self.mEntries.LastIndex)
		  For I As Integer = 0 To Self.mEntries.LastIndex
		    Pool(I) = New ArkSA.LootItemSetEntry(Self.mEntries(I))
		  Next
		  ...
		  For I As Integer = 1 To ChooseEntries
		    ...
		    Do
		      ...
		      Var SelectedEntry As ArkSA.LootItemSetEntry
	...
	#tag Property, Flags = &h1
		Protected mEntries() As ArkSA.LootItemSetEntry
	#tag EndProperty
