...
Protected Class SpawnPointLevel
	#tag Method, Flags = &h0
		Sub Constructor(Source As ArkSA.SpawnPointLevel)
	...
	#tag Method, Flags = &h0
		Shared Function FromSaveData(Dict As Dictionary) As ArkSA.SpawnPointLevel
		  If Dict Is Nil Then
		    Return Nil
		  End If
		  
		  If Dict.HasAllKeys("minLevel", "maxLevel", "difficulty") Then
		    Return New ArkSA.SpawnPointLevel(Dict.Value("minLevel"), Dict.Value("maxLevel"), Dict.Value("difficulty"))
		  ElseIf Dict.HasAllKeys("min_level", "max_level", "difficulty") Then
		    Return New ArkSA.SpawnPointLevel(Dict.Value("min_level"), Dict.Value("max_level"), Dict.Value("difficulty"))
		  ElseIf Dict.HasAllKeys("Min", "Max", "Diff") = False Then
		    Return New ArkSA.SpawnPointLevel(Dict.Value("Min"), Dict.Value("Max"), Dict.Value("Diff"))
		  Else
		    Return Nil
		  End If
		End Function
	...
	#tag Method, Flags = &h0
		Shared Function FromUserLevel(MinLevel As Integer, MaxLevel As Integer, Difficulty As Double) As ArkSA.SpawnPointLevel
		  // See note Formula
		  
		  Var Levels As New ArkSA.SpawnPointLevel(0, 0, 0)
	...
	#tag Method, Flags = &h0
		Function Operator_Compare(Other As ArkSA.SpawnPointLevel) As Integer
		  If Other = Nil Then
		    Return 1
		  End If
