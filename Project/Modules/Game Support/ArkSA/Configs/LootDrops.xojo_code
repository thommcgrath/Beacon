...
Implements Iterable, Beacon.BlueprintConsumer
	...
	#tag Method, Flags = &h0
		Sub Add(LootDrop As ArkSA.LootContainer, IncludeItemSets As Boolean)
		  ...
		  For Idx As Integer = 0 To Self.mOverrides.LastIndex
		    If Self.mOverrides(Idx).LootDropId = LootDrop.LootDropId Then
		      Self.mOverrides(Idx) = New ArkSA.LootDropOverride(LootDrop, IncludeItemSets)
		      Self.Modified = True
		      Return
		    End If
		  Next
		  
		  Self.mOverrides.Add(New ArkSA.LootDropOverride(LootDrop, IncludeItemSets))
		  Self.Modified = True
		End Sub
	#tag EndMethod
