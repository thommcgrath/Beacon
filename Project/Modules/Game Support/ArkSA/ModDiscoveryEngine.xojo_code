...
Protected Class ModDiscoveryEngine
	...
	#tag Method, Flags = &h21
		Private Sub mThread_Run(Sender As Thread)
		  ...
		  ConfigLines.Add("[" + ArkSA.HeaderServerSettings + "]")
		  ...
		  Var CustomConfig As New ArkSA.Configs.CustomContent
		  CustomConfig.GameUserSettingsIniContent = ConfigLines.Join(EndOfLine)
		  
		  Var Profile As New ArkSA.ServerProfile(Local.Identifier, "Mod Discovery")
		  ...
		  If ArkSA.DedicatedServer.Configure(Project, Profile, Self.mArkRoot, HostDir) = False Then
		  ...
		  Try
		    SteamCMD = ArkSA.DedicatedServer.SteamCMD(ServerFolder)
		  ...
		  For Each ModId As String In Self.mModIds
		    ...
		    Try
		      ArkSA.DedicatedServer.InstallMod(ModId, WorkshopFolder, ModsFolder)
		    Catch Err As RuntimeException
		      Self.Cleanup(HostDir)
		  ...
		  Var Executable As FolderItem = ArkSA.DedicatedServer.ShooterGameServer(ServerFolder)
	...
	#tag Method, Flags = &h0
		Sub Start(ArkRoot As FolderItem, ModIds() As String)
		  Var Executable As FolderItem = ArkSA.DedicatedServer.ShooterGameServer(ArkRoot)
