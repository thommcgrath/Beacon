...
Inherits Beacon.DataSource
	...
	#tag Method, Flags = &h21
		Private Sub ReplaceUserBlueprints()
		  ...
		  Self.BeginTransaction()
		  Self.SQLExecute("INSERT OR REPLACE INTO content_packs (content_pack_id, name, console_safe, default_enabled, is_local, last_update, marketplace, marketplace_id, game_id) VALUES (?1, ?2, ?3, ?4, ?5, ?6, '', '', ?7);", ArkSA.UserContentPackId, ArkSA.UserContentPackName, True, True, True, Beacon.FixedTimestamp, ArkSA.Identifier)
		  Self.CommitTransaction()
		End Sub
	#tag EndMethod
