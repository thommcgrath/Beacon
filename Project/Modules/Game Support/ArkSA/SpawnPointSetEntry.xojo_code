...
Implements Beacon.NamedItem
	#tag Method, Flags = &h0
		Function Clone() As ArkSA.SpawnPointSetEntry
		  Var Clone As New ArkSA.SpawnPointSetEntry(Self)
	...
	#tag Method, Flags = &h0
		Sub Constructor(Reference As ArkSA.BlueprintReference)
		  If Reference.Kind <> ArkSA.BlueprintReference.KindCreature Then
	...
	#tag Method, Flags = &h0
		Sub Constructor(Creature As ArkSA.Creature)
		  Self.Constructor()
		  Self.mCreatureRef = New ArkSA.BlueprintReference(Creature)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub Constructor(Source As ArkSA.SpawnPointSetEntry)
		  Self.Constructor()
		  
		  Self.mEntryId = Source.mEntryId
		  If (Source.mCreatureRef Is Nil) = False Then
		    Self.mCreatureRef = New ArkSA.BlueprintReference(Source.mCreatureRef)
		  End If
		  ...
		  For I As Integer = 0 To Source.mLevels.LastIndex
		    Self.mLevels(I) = New ArkSA.SpawnPointLevel(Source.mLevels(I))
		  Next
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Function Creature() As ArkSA.Creature
		  Var Blueprint As ArkSA.Blueprint = Self.mCreatureRef.Resolve()
	...
	#tag Method, Flags = &h0
		Function CreatureId() As String
		  Return Self.mCreatureRef.BlueprintId
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function CreatureReference() As ArkSA.BlueprintReference
		  Return Self.mCreatureRef
		End Function
	#tag EndMethod
		...
		Shared Function FromSaveData(Dict As Dictionary) As ArkSA.SpawnPointSetEntry
		  ...
		  Else
		    ...
		    If Creature Is Nil Then
		      Return Nil
		    End If
		    Entry = New ArkSA.SpawnPointSetEntry(Creature)
		  End If
		  ...
		  For Each LevelData As Dictionary In Levels
		    Var Level As ArkSA.SpawnPointLevel = ArkSA.SpawnPointLevel.FromSaveData(LevelData)
		    If Level <> Nil Then
	...
	#tag Method, Flags = &h0
		Function IndexOf(Level As ArkSA.SpawnPointLevel) As Integer
	...
	#tag Method, Flags = &h0
		Function Label() As String
		  Return Self.mCreatureRef.Label
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Function Level(AtIndex As Integer) As ArkSA.SpawnPointLevel
		  Return New ArkSA.SpawnPointLevel(Self.mLevels(AtIndex))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function LevelForDifficulty(Difficulty As Double, NilDefault As Boolean = False) As ArkSA.SpawnPointLevel
		  Var Candidate As ArkSA.SpawnPointLevel
		  For Each Level As ArkSA.SpawnPointLevel In Self.mLevels
		  ...
		  If Candidate = Nil And NilDefault = False Then
		    Candidate = ArkSA.SpawnPointLevel.FromUserLevel(1.0 * Difficulty, 30.0 * Difficulty, Difficulty)
		  End If
		  Return Candidate
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function LevelRangeForDifficulty(Difficulty As Double, OffsetBeforeMultiplier As Boolean) As Beacon.Range
		  If Self.HasCustomLevelRange Then
		    ...
		    Var Levels() As ArkSA.SpawnPointLevel
		    ...
		    Var LowDefinition, HighDefinition As ArkSA.SpawnPointLevel
	...
	#tag Method, Flags = &h0
		Function Levels() As ArkSA.SpawnPointLevel()
		  Var Arr() As ArkSA.SpawnPointLevel
		  Arr.ResizeTo(Self.mLevels.LastIndex)
		  For I As Integer = 0 To Self.mLevels.LastIndex
		    Arr(I) = New ArkSA.SpawnPointLevel(Self.mLevels(I))
		  Next
		  Return Arr
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function MutableClone() As ArkSA.MutableSpawnPointSetEntry
		  Var Clone As New ArkSA.MutableSpawnPointSetEntry(Self)
	...
	#tag Method, Flags = &h0
		Function MutableVersion() As ArkSA.MutableSpawnPointSetEntry
		  Return New ArkSA.MutableSpawnPointSetEntry(Self)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Operator_Compare(Other As ArkSA.SpawnPointSetEntry) As Integer
	...
	#tag Property, Flags = &h1
		Protected mCreatureRef As ArkSA.BlueprintReference
	#tag EndProperty
	...
	#tag Property, Flags = &h1
		Protected mLevels() As ArkSA.SpawnPointLevel
	#tag EndProperty
