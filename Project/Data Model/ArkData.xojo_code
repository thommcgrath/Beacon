...
Implements Beacon.DataSource
	...
	#tag Method, Flags = &h21
		Private Function Prepare(SQLString As String) As SQLitePreparedStatement
		  Dim Statement As SQLitePreparedStatement = Super.Prepare(SQLString)
		  If Self.Error Then
		    Self.HandleError(SQLString, Self.ErrorCode, Self.ErrorMessage)
		    Return Nil
		  End If
		  Return Statement
		End Function
	...
	#tag Method, Flags = &h21
		Private Sub SQLExecute(Statement As SQLitePreparedStatement, ParamArray Values() As Variant)
		  ...
		  If Self.Error Then
		    Self.HandleError("Statement", Self.ErrorCode, Self.ErrorMessage)
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function SQLSelect(SQLString As String) As RecordSet
		  Dim RS As RecordSet = Super.SQLSelect(SQLString)
		  If Self.Error Then
		    Self.HandleError(SQLString, Self.ErrorCode, Self.ErrorMessage)
		    Return Nil
		  End If
