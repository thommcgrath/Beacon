...
Begin BeaconDialog ArkModDiscoveryDialog
   ...
   Visible         =   True
   Width           =   600
   Begin UITweaks.ResizedPushButton ArkPathButton
   ...
   Begin UITweaks.ResizedLabel ModsLabel
      ...
      Width           =   115
   End
   Begin UITweaks.ResizedTextField ArkPathField
      AllowAutoDeactivate=   True
      AllowFocusRing  =   True
      AllowSpellChecking=   False
      AllowTabs       =   False
      ...
      Width           =   341
   End
   Begin UITweaks.ResizedLabel ArkPathLabel
      AllowAutoDeactivate=   True
      Bold            =   False
      Enabled         =   True
      FontName        =   "System"
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  Self.ArkPathField.Text = Preferences.ArkSteamPath
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, ForceModIds() As String) As Ark.ModDiscoverySettings
		  Var Win As New ArkModDiscoveryDialog(String.FromArray(ForceModIds, ","))
		  Win.ShowModal(Parent)
		  
		  Var Settings As Ark.ModDiscoverySettings = Win.mSettings
		  Win.Close
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, ParamArray ForceModIds() As String) As Ark.ModDiscoverySettings
	...
	#tag Property, Flags = &h21
		Private mSettings As Ark.ModDiscoverySettings
	#tag EndProperty
...
#tag Events ArkPathButton
	#tag Event
		Sub Pressed()
		  Var InitialFolder As FolderItem
		  Try
		    InitialFolder = New FolderItem(Self.ArkPathField.Text, FolderItem.PathModes.Native)
		  ...
		  Self.ArkPathField.Text = SelectedFolder.NativePath
		End Sub
	#tag EndEvent
#tag EndEvents
#tag Events ActionButton
	#tag Event
		Sub Pressed()
		  ...
		  Try
		    ArkFolder = New FolderItem(Self.ArkPathField.Text, FolderItem.PathModes.Native)
		  Catch Err As RuntimeException
