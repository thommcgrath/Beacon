...
#tag WindowCode
	...
	#tag Event
		Function ParsingFinished(Project As Ark.Project) As Boolean
		  If Project.HasConfigGroup(Ark.Configs.NameEngramControl) = False Then
		    Return False
		  End If
		  
		  Var NewGroup As Ark.Configs.EngramControl = Ark.Configs.EngramControl(Project.ConfigGroup(Ark.Configs.NameEngramControl))
		  Var CurrentGroup As Ark.Configs.EngramControl = Self.Config(False)
		  Var OldEngramRows As Dictionary = Self.PrepareEngramComparison()
		  Var Groups(1) As Ark.ConfigGroup
		  Groups(0) = CurrentGroup
		  Groups(1) = NewGroup
		  
		  Var MergedGroup As Ark.ConfigGroup = Ark.Configs.Merge(Groups, False)
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As Ark.Configs.EngramControl
		  Return Ark.Configs.EngramControl(Super.Config(ForWriting))
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Shared Function EngramMatchesFilter(Config As Ark.Configs.EngramControl, Engram As Ark.Engram, Filter As String) As Boolean
	...
	#tag Method, Flags = &h21
		Private Sub SetupEngramsList(SelectEngrams() As Ark.Engram = Nil)
		  ...
		  Var Engrams() As Ark.Engram = Beacon.Merge(Config.Engrams, Ark.DataSource.Pool.Get(False).GetEngramEntries("", Self.Project.ContentPacks, Nil))
	...
	#tag Constant, Name = kPointsClipboardType, Type = String, Dynamic = False, Default = \"com.thezaz.beacon.ark.unlockpointoverride", Scope = Private
	#tag EndConstant
...
#tag Events EngramList
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  ...
		  Var Config As Ark.Configs.EngramControl = Self.Config(True)
		  For Each Member As Ark.Engram In Members
	...
	#tag Event
		Sub PerformCopy(Board As Clipboard)
		  ...
		  For Idx As Integer = 0 To Me.LastRowIndex
		    ...
		    Var Engram As Ark.Engram = Me.RowTagAt(Idx)
		    Engrams.Add(Engram)
		  Next
