...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub Profile(Assigns Profile As Beacon.ServerProfile)
		  Var Items(0) As Beacon.ServerProfile
		  Items(0) = Profile
		  Self.Profiles = Items
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function Profiles() As Beacon.ServerProfile()
		  Return Self.mProfiles
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub Profiles(Assigns Items() As Beacon.ServerProfile)
	...
	#tag Method, Flags = &h0
		Sub RefreshUI()
		  Var Profile As Beacon.ServerProfile = Self.Profile
	...
	#tag Method, Flags = &h21
		Private Sub UpdateConfigSetUI()
		  ...
		  If Document.ConfigSetCount > 1 Then
		    ...
		    Var Profile As Beacon.ServerProfile = Self.Profile
		    If (Profile Is Nil) = False Then
		      States = Profile.ConfigSetStates(Document)
		    End If
		  ...
		  Else
		    Self.ConfigSetField.Text = Beacon.Document.BaseConfigSetName
...
#tag Events ConfigSetChooseButton
	#tag Event
		Sub Action()
		  Var MainProfile As Beacon.ServerProfile = Self.Profile
		  If MainProfile Is Nil Then
		    Return
		  End If
		  
		  Var States() As Beacon.ConfigSetState = MainProfile.ConfigSetStates(Self.Document)
