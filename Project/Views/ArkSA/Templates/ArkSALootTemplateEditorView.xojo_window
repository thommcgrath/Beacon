...
#tag WindowCode
	...
	#tag Event
		Sub Opening()
		  ...
		  Var AllMaps() As ArkSA.Map = ArkSA.Maps.All
		  Var MapNames() As String
		  For Each Map As ArkSA.Map In AllMaps
	...
	#tag Method, Flags = &h21
		Private Sub AddEntriesToList(Entries() As ArkSA.LootTemplateEntry)
		  Var Maps() As ArkSA.Map = Self.FilteredMaps()
		  For Each Entry As ArkSA.LootTemplateEntry In Entries
	...
	#tag Method, Flags = &h0
		Sub Constructor(Template As ArkSA.LootTemplate, SourceFile As FolderItem = Nil)
		  Self.mTemplate = New ArkSA.MutableLootTemplate(Template)
		...
		Private Sub DeleteSelectedEntries(Warn As Boolean)
		  ...
		  For I As Integer = Self.ContentsList.RowCount - 1 DownTo 0
		    If Not Self.ContentsList.RowSelectedAt(I) Then
		      Continue
		    End If
		    Var Entry As ArkSA.LootTemplateEntry = Self.ContentsList.RowTagAt(I)
	...
	#tag Method, Flags = &h21
		Private Sub EditSelectedEntries()
		  Var Entries() As ArkSA.LootItemSetEntry
		  For I As Integer = 0 To ContentsList.RowCount - 1
		    If Not ContentsList.RowSelectedAt(I) Then
		      Continue
		    End If
		    
		    Var Entry As ArkSA.MutableLootTemplateEntry = ContentsList.RowTagAt(I)
		    Entries.Add(Entry)
		  Next
		  ...
		  Var NewEntries() As ArkSA.LootItemSetEntry = ArkSALootEntryEditor.Present(Self.TrueWindow, Entries)
		  If NewEntries Is Nil Or NewEntries.Count <> Entries.Count Then
		    Return
		  End If
		  
		  Var Maps() As ArkSA.Map = Self.FilteredMaps()
		  For Each NewEntry As ArkSA.LootItemSetEntry In NewEntries
		    Var OriginalEntry As ArkSA.LootTemplateEntry
		    ...
		    Var Item As New ArkSA.MutableLootTemplateEntry(NewEntry)
		    ...
		    For Idx As Integer = 0 To ContentsList.RowCount - 1
		      If ArkSA.LootTemplateEntry(ContentsList.RowTagAt(Idx)).EntryId = OriginalEntry.EntryId Then
	...
	#tag Method, Flags = &h21
		Private Function FilteredMaps() As ArkSA.Map()
	...
	#tag Method, Flags = &h21
		Private Sub PutEntryInRow(Entry As ArkSA.LootTemplateEntry, Index As Integer, Maps() As ArkSA.Map, SelectIt As Boolean = False)
		  If Index = -1 Then
		    Self.ContentsList.AddRow("")
	...
	#tag Method, Flags = &h21
		Private Function SelectedEntries() As ArkSA.MutableLootTemplateEntry()
		  Var Entries() As ArkSA.MutableLootTemplateEntry
		  For I As Integer = Self.ContentsList.RowCount - 1 DownTo 0
	...
	#tag Method, Flags = &h21
		Private Sub ShowAddDialog()
		  Var Entries() As ArkSA.LootItemSetEntry = ArkSALootEntryEditor.Present(Self.TrueWindow)
		  ...
		  For Each Entry As ArkSA.LootItemSetEntry In Entries
		    Var Item As New ArkSA.LootTemplateEntry(Entry)
	...
	#tag Method, Flags = &h21
		Private Sub ShowModifierEditor(Edit As Boolean)
		  ...
		  End If
		  If ArkSALootTemplateModifierEditor.Present(Self, Self.mTemplate, EditID) Then
		    Self.UpdateUI
		    Self.Modified = True
		  End If
		End Sub
	...
	#tag Method, Flags = &h21
		Private Sub UpdateUI()
		  ...
		  For I As Integer = 0 To Self.ContentsList.RowCount - 1
		    If Self.ContentsList.RowSelectedAt(I) Then
		      Var Entry As ArkSA.MutableLootTemplateEntry = Self.ContentsList.RowTagAt(I)
	...
	#tag Method, Flags = &h21
		Private Sub UpdateUI(SelectedEntries() As ArkSA.LootTemplateEntry)
		  Var SelectedEntryIds() As String
		  ...
		  For Each Entry As ArkSA.LootTemplateEntry In SelectedEntries
	...
	#tag Method, Flags = &h21
		Private Sub UpdateUI(SelectedEntryIds() As String)
		  ...
		  Var Mask As UInt64 = Preferences.ArkSALastTemplateMapFilter
		  Var AllMaps() As ArkSA.Map = ArkSA.Maps.All
		  ...
		  For Each Entry As ArkSA.LootTemplateEntry In Self.mTemplate
		  ...
		  Var Selectors() As Beacon.TemplateSelector = Beacon.CommonData.Pool.Get(False).GetTemplateSelectors("", ArkSA.Identifier)
	...
	#tag Property, Flags = &h21
		Private mTemplate As ArkSA.MutableLootTemplate
	#tag EndProperty
	...
	#tag Event
		Sub CellAction(row As Integer, column As Integer)
		  ...
		  Case Self.ColumnIncluded
		    ...
		    Var Entry As ArkSA.MutableLootTemplateEntry = Me.RowTagAt(Row)
		    Var Maps() As ArkSA.Map = Self.FilteredMaps
		    For Each Map As ArkSA.Map In Maps
	...
	#tag Event
		Function ContextualMenuItemSelected(HitItem As DesktopMenuItem) As Boolean
		  ...
		  Case "createblueprintentry"
		    Var Entries() As ArkSA.LootTemplateEntry
		    ...
		    Var CreatedEntries() As ArkSA.LootTemplateEntry = Self.mTemplate.AddBlueprintEntries(Entries)
		  ...
		  Case "matchavailability"
		    ...
		    For Idx As Integer = 0 To Me.LastRowIndex
		      ...
		      Var Entry As ArkSA.MutableLootTemplateEntry = Me.RowTagAt(Idx)
...
#tag Events MapSelector
	#tag Event
		Sub Pressed()
		  ...
		  Var Maps() As ArkSA.Map = Self.FilteredMaps
		  Preferences.ArkSALastTemplateMapFilter = Maps.Mask
		  
		  For I As Integer = ContentsList.RowCount - 1 DownTo 0
		    Var Entry As ArkSA.LootTemplateEntry = ContentsList.RowTagAt(I)
