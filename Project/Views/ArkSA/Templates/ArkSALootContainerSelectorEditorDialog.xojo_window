#tag DesktopWindow
Begin BeaconDialog ArkSALootContainerSelectorEditorDialog
   Backdrop        =   0
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Function CreateTemplateSelector() As Beacon.TemplateSelector
		  ...
		  Return New Beacon.TemplateSelector(SelectorUUID, Label, ArkSA.Identifier, Language, Code)
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, TemplateSelector As Beacon.TemplateSelector = Nil) As Beacon.TemplateSelector
		  ...
		  If (TemplateSelector Is Nil) = False And TemplateSelector.GameId <> ArkSA.Identifier Then
		    // Can't edit this one
		    Return Nil
		  End If
		  
		  Var Win As New ArkSALootContainerSelectorEditorDialog(TemplateSelector)
	...
	#tag Method, Flags = &h21
		Private Function RunTest(Silent As Boolean, TemplateSelector As Beacon.TemplateSelector = Nil) As Boolean
		  ...
		  If LootSelector.TestCode(Message) Then
		    Var Sources() As ArkSA.LootContainer = ArkSA.DataSource.Pool.Get(False).GetLootContainers("", New Beacon.StringList, "", True)
