#tag DesktopWindow
Begin BeaconContainer ArkSASpawnPointSetEditor
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Function SpawnSet() As ArkSA.MutableSpawnPointSet
		  If (Self.mRef Is Nil) = False And (Self.mRef.Value Is Nil) = False Then
		    Return ArkSA.MutableSpawnPointSet(Self.mRef.Value)
		  End If
		End Function
	#tag EndMethod

	#tag Method, Flags = &h0
		Sub SpawnSet(Assigns Set As ArkSA.MutableSpawnPointSet)
		  ...
		  If Set Is Nil Then
		    Self.mRef = Nil
		    Self.UpdateUI(New ArkSA.MutableSpawnPointSet)
		    Return
		  End If
	...
	#tag Method, Flags = &h21
		Private Sub UpdateEntriesList(Set As ArkSA.SpawnPointSet, SelectEntries() As ArkSA.SpawnPointSetEntry = Nil)
		  ...
		  If SelectEntries = Nil Then
		    For I As Integer = 0 To List.RowCount - 1
		      If List.RowSelectedAt(I) Then
		        Var Entry As ArkSA.SpawnPointSetEntry = List.RowTagAt(I)
		        SelectedEntries.Add(Entry.EntryId)
		      End If
		    Next
		  Else
		    For Each Entry As ArkSA.SpawnPointSetEntry In SelectEntries
		      SelectedEntries.Add(Entry.EntryId)
		    Next
		  End If
		  ...
		  For RowIndex As Integer = 0 To Bound
		    Var Entry As ArkSA.SpawnPointSetEntry = Set.Entry(RowIndex)
	...
	#tag Method, Flags = &h21
		Private Sub UpdateReplacementsList(Set As ArkSA.SpawnPointSet, SelectCreatures() As ArkSA.BlueprintReference = Nil)
		  ...
		  If SelectCreatures Is Nil Then
		    For I As Integer = 0 To Self.ReplaceList.RowCount - 1
		      If Self.ReplaceList.RowSelectedAt(I) Then
		        Var Reference As ArkSA.BlueprintReference = Self.ReplaceList.RowTagAt(I)
		  ...
		  Var ReplacedCreatures() As ArkSA.BlueprintReference = Set.ReplacedCreatureRefs
		  ...
		  For RowIndex As Integer = 0 To ReplacedCreatures.LastIndex
		    Var ReplacedCreature As ArkSA.BlueprintReference = ReplacedCreatures(RowIndex)
		    Var ReplacementCreatures() As ArkSA.BlueprintReference = Set.ReplacementCreatures(ReplacedCreature)
		    
		    Var TotalWeight As Double
		    For Each ReplacementCreature As ArkSA.BlueprintReference In ReplacementCreatures
	...
	#tag Method, Flags = &h21
		Private Sub UpdateUI(Set As ArkSA.SpawnPointSet)
		  ...
		  If Set.ColorSetClass.IsEmpty Then
		    FoundColorSet = True
		  End If
		  Var ColorSets() As ArkSA.CreatureColorSet = ArkSA.DataSource.Pool.Get(False).GetCreatureColorSets()
		  For Each ColorSet As ArkSA.CreatureColorSet In ColorSets
...
#tag Events EntryAddButton
	#tag Event
		Sub Pressed()
		  Var Set As ArkSA.MutableSpawnPointSet = Self.SpawnSet
		  Var Entry As ArkSA.MutableSpawnPointSetEntry = ArkSASpawnPointCreatureDialog.Present(Self, Self.Project, Set)
		  If Entry = Nil Then
		    Return
		  End If
		  ...
		  Var Entries(0) As ArkSA.MutableSpawnPointSetEntry
		  Entries(0) = Entry
...
#tag Events EntriesList
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  ...
		  Var EntriesToDelete() As ArkSA.SpawnPointSetEntry
		  For I As Integer = 0 To Bound
		    If Me.RowSelectedAt(I) = False Then
		      Continue
		    End If
		    
		    Var Entry As ArkSA.SpawnPointSetEntry = Me.RowTagAt(I)
		  ...
		  For Each Entry As ArkSA.SpawnPointSetEntry In EntriesToDelete
		...
		Sub PerformPaste(Board As Clipboard)
		  ...
		  If Contents.IsNull = False Then
		    Try
		      ...
		      Var Set As ArkSA.MutableSpawnPointSet = Self.SpawnSet
		      Var SelectEntries() As ArkSA.SpawnPointSetEntry
		      For Each Dict As Dictionary In Dicts
		        Var Entry As ArkSA.SpawnPointSetEntry = ArkSA.SpawnPointSetEntry.FromSaveData(Dict)
	...
	#tag Event
		Sub PerformEdit()
		  Var Entries() As ArkSA.MutableSpawnPointSetEntry
		  For I As Integer = 0 To Me.RowCount - 1
		    If Me.RowSelectedAt(I) = False Then
		      Continue
		    End If
		  ...
		  Var Set As ArkSA.MutableSpawnPointSet = Self.SpawnSet
		  Var UpdatedEntries() As ArkSA.MutableSpawnPointSetEntry = ArkSASpawnPointCreatureDialog.Present(Self, Self.Project, Set, Entries)
		  If UpdatedEntries = Nil Or UpdatedEntries.LastIndex = -1 Then
		    Return
		  End If
		  
		  For Each Entry As ArkSA.MutableSpawnPointSetEntry In Entries
		    Set.Remove(Entry)
		  Next
		  For Each Entry As ArkSA.MutableSpawnPointSetEntry In UpdatedEntries
		    Set.Append(Entry)
		  Next
	...
	#tag Event
		Function RowComparison(row1 as Integer, row2 as Integer, column as Integer, ByRef result as Integer) As Boolean
		  ...
		  Var Entry1 As ArkSA.SpawnPointSetEntry = Me.RowTagAt(Row1)
		  Var Entry2 As ArkSA.SpawnPointSetEntry = Me.RowTagAt(Row2)
...
#tag Events ReplaceAddButton
	#tag Event
		Sub Pressed()
		  ...
		  Var Creature As ArkSA.BlueprintReference = ArkSASpawnPointReplacementsDialog.Present(Self, Self.Project.ContentPacks, Set)
		  If Creature = Nil Then
		    Return
		  End If
		  
		  Var Creatures(0) As ArkSA.BlueprintReference
...
#tag Events ReplaceList
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  Var Bound As Integer = Me.RowCount - 1
		  Var CreaturesToDelete() As ArkSA.BlueprintReference
		  For I As Integer = Bound DownTo 0
		    If Me.RowSelectedAt(I) = False Then
		      Continue
		    End If
		    
		    Var Creature As ArkSA.BlueprintReference = Me.RowTagAt(I)
		  ...
		  Var Set As ArkSA.MutableSpawnPointSet = Self.SpawnSet
		  Var Changed As Boolean
		  For Each FromCreature As ArkSA.BlueprintReference In CreaturesToDelete
		    Var Replacements() As ArkSA.BlueprintReference = Set.ReplacementCreatures(FromCreature)
		    For Each ToCreature As ArkSA.BlueprintReference In Replacements
	...
	#tag Event
		Sub PerformCopy(Board As Clipboard)
		  Var Items() As Dictionary
		  Var Set As ArkSA.SpawnPointSet = Self.SpawnSet
		  For I As Integer = 0 To Me.RowCount - 1
		    If Not Me.RowSelectedAt(I) Then
		      Continue
		    End If
		    
		    Var FromCreatureRef As ArkSA.BlueprintReference = Me.RowTagAt(I)
		    Var ReplacementRefs() As ArkSA.BlueprintReference = Set.ReplacementCreatures(FromCreatureRef)
	...
	#tag Event
		Sub PerformPaste(Board As Clipboard)
		  ...
		  Try
		    ...
		    For Each Dict As Dictionary In Dicts
		      Var FromCreatureRef As ArkSA.BlueprintReference = ArkSA.BlueprintReference.FromSaveData(Dictionary(Dict.Value("creature")))
		      ...
		      For Each Replacement As Dictionary In Replacements
		        Var ToCreatureRef As ArkSA.BlueprintReference = ArkSA.BlueprintReference.FromSaveData(Dictionary(Replacement.Value("creature")))
	...
	#tag Event
		Sub PerformEdit()
		  Var TargetCreatureRef As ArkSA.BlueprintReference = Self.ReplaceList.RowTagAt(Self.ReplaceList.SelectedRowIndex)
		  If TargetCreatureRef Is Nil Then
		    Return
		  End If
		  
		  Var CreatureRef As ArkSA.BlueprintReference = ArkSASpawnPointReplacementsDialog.Present(Self, Self.Project.ContentPacks, Self.SpawnSet, TargetCreatureRef)
		  If CreatureRef Is Nil Then
		    Return
		  End If
		  
		  Var CreatureRefs(0) As ArkSA.BlueprintReference
