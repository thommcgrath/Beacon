#tag DesktopWindow
Begin BeaconDialog ArkSASpawnPointCreatureDialog
   Backdrop        =   0
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  ...
		  ElseIf Self.mMultiEditMode Then
		    Var Names() As String
		    For Each Entry As ArkSA.MutableSpawnPointSetEntry In Self.mEntries
	...
	#tag Method, Flags = &h0
		Sub Constructor(Project As ArkSA.Project, Entries() As ArkSA.MutableSpawnPointSetEntry, OffsetBeforeMultiplier As Boolean)
		  ...
		  If Entries.LastIndex > 0 Then
		    ...
		    Var CommonCreature As ArkSA.Creature = Entries(0).Creature
	...
	#tag Method, Flags = &h21
		Private Function CreateEntry() As ArkSA.MutableSpawnPointSetEntry
		  // For the purposes of this method, we don't need a creature, but the class does. So give it one that will always exist. A dodo.
		  Var Entry As New ArkSA.MutableSpawnPointSetEntry(ArkSA.DataSource.Pool.Get(False).GetCreature("08d6f1c5-6b8a-48b0-9232-e2705864c87c"))
		  
		  If Self.LevelOverrideMinField.Text <> "" And Self.LevelOverrideMaxField.Text <> "" Then
		    Entry.Append(ArkSA.SpawnPointLevel.FromUserLevel(Self.LevelOverrideMinField.DoubleValue, Self.LevelOverrideMaxField.DoubleValue, Self.mDifficulty))
		  End If
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Project As ArkSA.Project, Set As ArkSA.SpawnPointSet, Entry As ArkSA.MutableSpawnPointSetEntry = Nil) As ArkSA.MutableSpawnPointSetEntry
		  Var Entries() As ArkSA.MutableSpawnPointSetEntry
		  If Entry <> Nil Then
		    Entries.Add(Entry)
		  End If
		  
		  Var Results() As ArkSA.MutableSpawnPointSetEntry = Present(Parent, Project, Set, Entries)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Project As ArkSA.Project, Set As ArkSA.SpawnPointSet, Entries() As ArkSA.MutableSpawnPointSetEntry) As ArkSA.MutableSpawnPointSetEntry()
		  Var NewEntries() As ArkSA.MutableSpawnPointSetEntry
		  
		  If Parent = Nil Then
		    Return NewEntries
		  End If
		  
		  Var Win As New ArkSASpawnPointCreatureDialog(Project, Entries, Set.LevelOffsetBeforeMultiplier)
	...
	#tag Method, Flags = &h21
		Private Sub UpdateEffectiveLevel()
		  ...
		  Var Entry As ArkSA.MutableSpawnPointSetEntry = Self.CreateEntry
	...
	#tag Property, Flags = &h21
		Private mEntries() As ArkSA.MutableSpawnPointSetEntry
	#tag EndProperty
...
#tag Events CreatureChooseButton
	#tag Event
		Sub Pressed()
		  ...
		  Var Creatures() As ArkSA.Creature = ArkSABlueprintSelectorDialog.Present(Self, "", Exclude, Self.mMods, ArkSABlueprintSelectorDialog.SelectModes.Single)
...
#tag Events ActionButton
	#tag Event
		Sub Pressed()
		  ...
		  If Self.mEntries.LastIndex = -1 Then
		    Self.mEntries.Add(New ArkSA.MutableSpawnPointSetEntry(Self.mTargetCreature))
		  End If
		  ...
		  For Each Entry As ArkSA.MutableSpawnPointSetEntry In Self.mEntries
		    ...
		    If Self.mEditedFields.IndexOf(Self.LevelOverrideMinField) > -1 Or Self.mEditedFields.IndexOf(Self.LevelOverrideMaxField) > -1 Then
		      ...
		      Entry.LevelCount = 0
		      Entry.Append(ArkSA.SpawnPointLevel.FromUserLevel(MinLevel, MaxLevel, Self.mDifficulty))
