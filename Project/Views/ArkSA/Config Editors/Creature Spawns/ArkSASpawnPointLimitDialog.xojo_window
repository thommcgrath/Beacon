#tag DesktopWindow
Begin BeaconDialog ArkSASpawnPointLimitDialog
   Backdrop        =   0
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub Constructor(Mods As Beacon.StringList, Limit As NullableDouble, SelectedCreatures() As ArkSA.BlueprintReference, DefinedCreatures() As ArkSA.BlueprintReference, CreaturesInSpawnPoint() As ArkSA.BlueprintReference)
		  ...
		  Var SelectableCreatures() As ArkSA.BlueprintReference
		  ...
		  End If 
		  For Each CreatureRef As ArkSA.BlueprintReference In DefinedCreatures
		  ...
		  For Each CreatureRef As ArkSA.BlueprintReference In CreaturesInSpawnPoint
		    If CreatureRef Is Nil Then
		      Continue
		    End If
		  ...
		  Var AllCreatures() As ArkSA.Creature = ArkSA.DataSource.Pool.Get(False).GetCreatures("", Mods, Nil)
		  For Each Creature As ArkSA.Creature In AllCreatures
		    If IsNull(Creature) Or Map.HasKey(Creature.CreatureId) Then
		      Continue
		    End If
		    
		    SelectableCreatures.Add(New ArkSA.BlueprintReference(Creature))
		  Next
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Mods As Beacon.StringList, Limit As NullableDouble, SelectedCreatures() As ArkSA.BlueprintReference, DefinedCreatures() As ArkSA.BlueprintReference, CreaturesInSpawnPoint() As ArkSA.BlueprintReference) As NullableDouble
		  If Parent Is Nil Then
		    Return Nil
		  End If
		  
		  Var Dialog As New ArkSASpawnPointLimitDialog(Mods, Limit, SelectedCreatures, DefinedCreatures, CreaturesInSpawnPoint)
	...
	#tag Property, Flags = &h21
		Private mSelectableCreatures() As ArkSA.BlueprintReference
	#tag EndProperty
...
#tag Events HelpButton
	#tag Event
		Sub Pressed()
		  ...
		  Call ArkSA.DataSource.Pool.Get(False).GetConfigHelp(ArkSA.Configs.NameCreatureSpawns, Title, Body, HelpURL)
	...
	#tag Event
		Sub Opening()
		  If Self.mDisableSelection Then
		    ...
		    Me.Enabled = False
		  Else
		    For Each CreatureRef As ArkSA.BlueprintReference In Self.mSelectableCreatures
		      If CreatureRef Is Nil Then
		        #if TargetMacOS
		          Me.AddSeparator
		        #endif
