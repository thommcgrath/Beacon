#tag DesktopWindow
Begin BeaconWindow ArkSASpawnSimulatorWindow
   Backdrop        =   0
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub UpdateSpawnPointView()
		  ...
		  Var PopFigures As ArkSA.PopulationFigures
		  
		  If (Self.mSelectedPoint Is Nil) = False Then
		    Var Figures() As ArkSA.PopulationFigures = ArkSA.DataSource.Pool.Get(False).GetPopulationFigures(Self.mSelectedPoint)
	...
	#tag Property, Flags = &h21
		Private mSelectedPoint As ArkSA.SpawnPoint
	#tag EndProperty
	...
	#tag ComputedProperty, Flags = &h21
		...
		#tag Setter
			...
			End Set
		#tag EndSetter
		Private SelectedPoint As ArkSA.SpawnPoint
	#tag EndComputedProperty
...
#tag Events MapMenu
	#tag Event
		Sub Opening()
		  Var Maps() As ArkSA.Map = Self.mProject.Maps
