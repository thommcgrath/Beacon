#tag DesktopWindow
Begin BeaconDialog ArkSABulkSpawnEditWindow
   Backdrop        =   0
...
#tag WindowCode
	#tag Method, Flags = &h21
		Private Sub Constructor(Config As ArkSA.Configs.SpawnPoints, Mods As Beacon.StringList, Mask As UInt64, DifficultyValue As Double)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Config As ArkSA.Configs.SpawnPoints, Mods As Beacon.StringList, Mask As UInt64, DifficultyValue As Double) As Boolean
		  If Parent Is Nil Then
		    Return False
		  End If
		  
		  Var Win As New ArkSABulkSpawnEditWindow(Config, Mods, Mask, DifficultyValue)
		...
		Private Sub ProcessSpawnPoint(Point As ArkSA.MutableSpawnPointOverride)
		  For SetIdx As Integer = Point.LastIndex DownTo 0
		    ...
		    For EntryIdx As Integer = MutableSet.LastIndex DownTo 0
		      Var Entry As ArkSA.MutableSpawnPointSetEntry = MutableSet.Entry(EntryIdx).MutableVersion
		    ...
		    If MutableSet.Count > 0 Then
		      If MutableSet.ReplacesCreatures And (Self.mReplacesCreatures Or Self.mRemovesCreatures) Then
		        Var ReplacedRefs() As ArkSA.BlueprintReference = MutableSet.ReplacedCreatureRefs
		        For Each FromCreatureRef As ArkSA.BlueprintReference In ReplacedRefs
		          Var OriginalFromCreatureRef As ArkSA.BlueprintReference = FromCreatureRef
		          ...
		          Var ReplacementRefs() As ArkSA.BlueprintReference = MutableSet.ReplacementCreatures(OriginalFromCreatureRef)
		          For Each ToCreatureRef As ArkSA.BlueprintReference In ReplacementRefs
		            Var OriginalToCreatureRef As ArkSA.BlueprintReference = ToCreatureRef
	...
	#tag Method, Flags = &h21
		Private Sub UpdateEnabledState()
		  Var ColorsAllowed, LevelsAllowed, WeightAllowed, ReplaceAllowed, RemoveAllowed As Boolean = True
		  Var ColorsEnabled, LevelsEnabled, WeightEnabled, ReplaceEnabled As Boolean
		  ...
		  Else
		    ...
		    RemoveAllowed = Not (ColorsEnabled Or LevelsEnabled Or WeightEnabled Or ReplaceEnabled)
		  End If
	...
	#tag Property, Flags = &h21
		Private mConfig As ArkSA.Configs.SpawnPoints
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mCreatures() As ArkSA.Creature
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mLevelOverrides() As ArkSA.SpawnPointLevel
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mReplacementCreatureRef As ArkSA.BlueprintReference
	#tag EndProperty
...
#tag Events ChooseCreaturesButton
	#tag Event
		Sub Pressed()
		  Var SelectedCreatures() As ArkSA.Creature
		  For Idx As Integer = 0 To Self.CreatureList.LastRowIndex
		    SelectedCreatures.Add(Self.CreatureList.RowTagAt(Idx))
		  Next Idx
		  
		  Var AddedCreatures() As ArkSA.Creature = ArkSABlueprintSelectorDialog.Present(Self, "", SelectedCreatures, Self.mMods, ArkSABlueprintSelectorDialog.SelectModes.ExplicitMultiple)
		  For Each Creature As ArkSA.Creature In AddedCreatures
...
#tag Events ActionButton
	#tag Event
		Sub Pressed()
		  ...
		  Var WeightMultiplier As Double = 1.0
		  
		  If Self.ChangeColorsCheckbox.Value = False And Self.SetLevelsCheckbox.Value = False And Self.MultiplyWeightCheckbox.Value = False And Self.RemoveCreatureCheckbox.Value = False And Self.ReplaceWithCheckbox.Value = False Then
		  ...
		  Else
		    For RowIdx As Integer = 0 To Self.CreatureList.LastRowIndex
		      Var Creature As ArkSA.Creature = Self.CreatureList.RowTagAt(RowIdx)
		  ...
		  If Self.mChangeLevels Then
		    ...
		    Self.mLevelOverrides.Add(ArkSA.SpawnPointLevel.FromUserLevel(MinLevel, MaxLevel, Self.mDifficultyValue))
		  End If
...
#tag Events ColorsMenu
	#tag Event
		Sub Opening()
		  ...
		  Var Colors() As ArkSA.CreatureColorSet = ArkSA.DataSource.Pool.Get(False).GetCreatureColorSets()
		  For Each ColorSet As ArkSA.CreatureColorSet In Colors
...
#tag Events ChooseReplacementButton
	#tag Event
		Sub Pressed()
		  Var SelectedCreatures() As ArkSA.Creature
		  Var AddedCreatures() As ArkSA.Creature = ArkSABlueprintSelectorDialog.Present(Self, "", SelectedCreatures, Self.mMods, ArkSABlueprintSelectorDialog.SelectModes.Single)
		  If AddedCreatures.Count >= 1 Then
		    Self.mReplacementCreatureRef = New ArkSA.BlueprintReference(AddedCreatures(0))
		...
		Sub Run()
		  ...
		  For Each Creature As ArkSA.Creature In Self.mCreatures
		    ...
		    For Each Definition As ArkSA.SpawnPoint In Points
		      ...
		      Var Original As ArkSA.SpawnPointOverride = Self.mConfig.OverrideForSpawnPoint(Definition, ArkSA.SpawnPointOverride.ModeOverride)
		      If (Original Is Nil) = False Then
		        Continue
		      End If
		      
		      Var Mutable As New ArkSA.MutableSpawnPointOverride(Definition, ArkSA.SpawnPointOverride.ModeOverride, True)
		      ...
		      Var Append As ArkSA.SpawnPointOverride = Self.mConfig.OverrideForSpawnPoint(Definition, ArkSA.SpawnPointOverride.ModeAppend)
