...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub PresentLimitsDialog(SelectedCreatures() As ArkSA.Creature)
		  Var DefinedCreatures() As ArkSA.Creature
		  Var CommonLimit As NullableDouble
		  Var CreatureFilter As New Dictionary
		  
		  For Each Point As ArkSA.SpawnPoint In Self.mSpawnPoints
		    ...
		    For I As Integer = 0 To Bound
		      Var Set As ArkSA.SpawnPointSet = Point.Set(I)
		  ...
		  Var CreaturesInSpawnPoint() As ArkSA.Creature
		  ...
		  If Limit <> Nil Then
		    For Each Point As ArkSA.MutableSpawnPoint In Self.mSpawnPoints
		      For Each Creature As ArkSA.Creature In SelectedCreatures
	...
	#tag Method, Flags = &h0
		Function SpawnPoints() As ArkSA.SpawnPoint()
		  ...
		  For Idx As Integer = 0 To Self.mSpawnPoints.LastIndex
		    Points(Idx) = New ArkSA.SpawnPoint(Self.mSpawnPoints(Idx))
		  Next
		  Return Points
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub UpdateLimitsList(SelectCreatures() As ArkSA.Creature = Nil)
	...
	#tag Property, Flags = &h21
		Private mSpawnPoints() As ArkSA.MutableSpawnPoint
	#tag EndProperty
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  ...
		  For Each Organizer As ArkSA.SpawnSetOrganizer In Organizers
		    Var Points() As ArkSA.MutableSpawnPoint = Organizer.Points
		    For Each Point As ArkSA.MutableSpawnPoint In Points
		      Var Set As ArkSA.SpawnPointSet = Organizer.SetForPoint(Point)
		      Point.RemoveSet(Set)
		    Next
		  Next
...
#tag Events LimitsList
	...
	#tag Event
		Sub DoublePressed()
		  ...
		  For I As Integer = 0 To Self.LimitsList.RowCount - 1
		    If Self.LimitsList.RowSelectedAt(I) Then
		      SelectedCreatures.Add(ArkSA.DataSource.Pool.Get(False).GetCreature(Self.LimitsList.RowTagAt(I).StringValue))
		    End If
		  Next
		  Self.PresentLimitsDialog(SelectedCreatures)
		End Sub
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  ...
		  For I As Integer = 0 To Me.RowCount - 1
		    ...
		    Var Creature As ArkSA.Creature = ArkSA.DataSource.Pool.Get(False).GetCreature(Me.RowTagAt(I).StringValue)
		    If Creature Is Nil Then
		      Continue
		    End If
	...
	#tag Event
		Sub PerformCopy(Board As Clipboard)
		  ...
		  For I As Integer = 0 To Me.RowCount - 1
		    ...
		    Var Creature As ArkSA.Creature = ArkSA.DataSource.Pool.Get(False).GetCreature(UUID)
		    If Creature Is Nil Then
		      Continue
		    End If
