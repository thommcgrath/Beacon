#tag DesktopWindow
Begin ArkSAConfigEditor ArkSACustomConfigEditor Implements NotificationKit.Receiver
...
#tag WindowCode
	...
	#tag Event
		Function ParsingFinished(Project As ArkSA.Project) As Boolean
		  ...
		  Var SelfProject As ArkSA.Project = Self.Project
		  Var CreatedEditorNames() As String
		  Var Config As ArkSA.Configs.CustomContent = Self.Config(False)
		  Var Organizer As New ArkSA.ConfigOrganizer(ArkSA.ConfigFileGame, ArkSA.HeaderShooterGame, Config.GameIniContent)
		  Organizer.Add(ArkSA.ConfigFileGameUserSettings, ArkSA.HeaderServerSettings, Config.GameUserSettingsIniContent)
		  
		  For Each CreatedConfig As ArkSA.ConfigGroup In Project.ImplementedConfigs(Project.ActiveConfigSet)
		    Var InternalName As String = CreatedConfig.InternalName
		    If InternalName = ArkSA.Configs.NameCustomConfig Then
		      Continue
		    End If
		    
		    If ArkSA.Configs.ConfigUnlocked(InternalName, Identity) = False Then
		    ...
		    If SelfProject.HasConfigGroup(InternalName) Then
		      Var CurrentConfig As ArkSA.ConfigGroup = SelfProject.ConfigGroup(InternalName, False)
		      If (CurrentConfig Is Nil) = False Then
		        Var Configs(1) As ArkSA.ConfigGroup
		        Configs(0) = CurrentConfig
		        Configs(1) = CreatedConfig
		        
		        Var Merged As ArkSA.ConfigGroup = ArkSA.Configs.Merge(Configs, True)
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.Configs.CustomContent
		  Return ArkSA.Configs.CustomContent(Super.Config(ForWriting))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function CurrentHeader() As String
		  ...
		  Select Case Self.ActiveButtonName
		  Case "GameUserSettingsIniButton"
		    Header = ArkSA.HeaderServerSettings
		  Case "GameIniButton"
		    Header = ArkSA.HeaderShooterGame
		  End Select
	...
	#tag Method, Flags = &h21
		Private Sub ToggleEncryption()
		  Var Tag As String = ArkSA.Configs.CustomContent.EncryptedTag
	...
	#tag Method, Flags = &h21
		Private Sub UpdateAutoComplete()
		  ...
		  Var Configs() As ArkSA.ConfigOption = ArkSA.DataSource.Pool.Get(False).GetConfigOptions(CurrentFile, CurrentHeader, "", False)
...
#tag Events ConfigArea
	...
	#tag Event
		Sub DWellStart(Position as Integer, X as Integer, Y as Integer)
		  ...
		  Var Key As ArkSA.ConfigOption = ArkSA.DataSource.Pool.Get(False).GetConfigOption(Self.CurrentFile, Self.CurrentHeader, Line.Left(EqualsPosition))
