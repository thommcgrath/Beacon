#tag DesktopWindow
Begin ArkSAConfigEditor ArkSAStackSizesEditor
...
#tag WindowCode
	#tag Event
		Function ParsingFinished(Project As ArkSA.Project) As Boolean
		  ...
		  If Project Is Nil Or Project.HasConfigGroup(ArkSA.Configs.NameStackSizes) = False Then
		    Return True
		  End If
		  
		  Var OtherConfig As ArkSA.Configs.StackSizes = ArkSA.Configs.StackSizes(Project.ConfigGroup(ArkSA.Configs.NameStackSizes))
		  If OtherConfig = Nil Or OtherConfig.Count = CType(0, UInteger) Then
		    Return True
		  End If
		  
		  Var Config As ArkSA.Configs.StackSizes = Self.Config(True)
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.Configs.StackSizes
		  Return ArkSA.Configs.StackSizes(Super.Config(ForWriting))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub ShowAddOverride()
		  Var Config As ArkSA.Configs.StackSizes = Self.Config(False)
		  Var CurrentEngrams() As ArkSA.Engram = Config.Engrams
		  
		  Var NewEngrams() As ArkSA.Engram = ArkSABlueprintSelectorDialog.Present(Self, "Stackables", CurrentEngrams, Self.Project.ContentPacks, ArkSABlueprintSelectorDialog.SelectModes.ImpliedMultiple)
		  ...
		  For Each Engram As ArkSA.Engram In NewEngrams
		    ...
		    Else
		      Config.Override(Engram) = Min(Round(Engram.StackSize.DoubleValue * Config.GlobalMultiplier), ArkSA.Configs.StackSizes.MaximumQuantity)
		    End If
		  Next
	...
	#tag Method, Flags = &h21
		Private Sub ShowDuplicateOverride()
		  ...
		  Var NewEngrams() As ArkSA.Engram = ArkSABlueprintSelectorDialog.Present(Self, "Stackables", CurrentEngrams, Self.Project.ContentPacks, ArkSABlueprintSelectorDialog.SelectModes.ExplicitMultiple)
		  If NewEngrams = Nil Or NewEngrams.LastIndex = -1 Then
		    Return
		  End If
	...
	#tag Method, Flags = &h21
		Private Sub UpdateList()
		  ...
		  For I As Integer = 0 To Self.List.RowCount - 1
		    ...
		    Engrams.Add(ArkSA.Engram(Self.List.RowTagAt(I)).EngramId)
		  Next
		  Self.UpdateList(Engrams)
		End Sub
	#tag EndMethod
...
#tag Events List
	...
	#tag Event
		Sub CellAction(row As Integer, column As Integer)
		  ...
		  If Size > CType(ArkSA.Configs.StackSizes.MaximumQuantity, UInt64) Then
		    Size = ArkSA.Configs.StackSizes.MaximumQuantity
		    System.Beep
		    Self.ShowAlert("Stack size too high", "Ark has a maximum stack size of " + ArkSA.Configs.StackSizes.MaximumQuantity.ToString(Locale.Current, Self.NumberFormat) + ".")
		  End If
