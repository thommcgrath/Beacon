#tag DesktopWindow
Begin ArkSAServerViewContainer ArkSALocalServerView
	...
	#tag Method, Flags = &h21
		Private Function DetectConfigType(File As FolderItem) As String
		  ...
		  ElseIf File <> Nil Then
		    ...
		    Case ArkSA.ConfigFileGame, ArkSA.ConfigFileGameUserSettings
		      Return File.Name
		    End Select
		  End If
...
#tag Events GameUserSettingsIniChooseButton
	#tag Event
		Sub Pressed()
		  ...
		  Dialog.SuggestedFileName = ArkSA.ConfigFileGameUserSettings
		  ...
		  If DetectedType <> ArkSA.ConfigFileGameUserSettings And Self.ShowConfirm("The selected file does not appear to be a GameUserSettings.ini file.", "You can still choose this file if you really want, but its content does not look like a GameUserSettings.ini file. Proceed with caution.", "Use Anyway", "Cancel") = False Then
		    Return
		  End If
...
#tag Events GameIniChooseButton
	#tag Event
		Sub Pressed()
		  Var Dialog As New OpenFileDialog
		  Dialog.Filter = BeaconFileTypes.IniFile
		  Dialog.SuggestedFileName = ArkSA.ConfigFileGame
		  ...
		  Var DetectedType As String = Self.DetectConfigType(File)
		  If DetectedType <> ArkSA.ConfigFileGame And Self.ShowConfirm("The selected file does not appear to be a Game.ini file.", "You can still choose this file if you really want, but its content does not look like a Game.ini file. Proceed with caution.", "Use Anyway", "Cancel") = False Then
		    Return
		  End If
