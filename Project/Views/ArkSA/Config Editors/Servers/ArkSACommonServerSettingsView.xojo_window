#tag DesktopWindow
Begin BeaconContainer ArkSACommonServerSettingsView
...
#tag Events ConfigSetChooseButton
	#tag Event
		Sub Pressed()
		  Var MainProfile As ArkSA.ServerProfile = Self.Profile
...
#tag Events MessageOfTheDayArea
	#tag Event
		Sub TextChanged()
		  ...
		  For Each Profile As ArkSA.ServerProfile In Self.mProfiles
		    Profile.MessageOfTheDay = ArkSA.ArkML.FromRTF(Me.RTFData)
		  Next
		  Self.Modified = Self.Modified
		End Sub
	#tag EndEvent
...
#tag Events MapMenu
	#tag Event
		Sub Opening()
		  Me.AddRow("All Maps", ArkSA.Maps.UniversalMask)
