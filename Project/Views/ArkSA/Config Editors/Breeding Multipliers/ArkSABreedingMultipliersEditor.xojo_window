...
#tag WindowCode
	...
	#tag Event
		Sub SetupUI()
		  Var Config As ArkSA.Configs.BreedingMultipliers = Self.Config(False)
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.Configs.BreedingMultipliers
		  Return ArkSA.Configs.BreedingMultipliers(Super.Config(ForWriting))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub UpdateBaselineRates()
		  ...
		  If Self.mComputeSinglePlayer Then
		    MatingIntervalMultiplier = MatingIntervalMultiplier * ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("SPMatingIntervalMultiplier", 0.125)
		    EggHatchSpeedMultiplier = EggHatchSpeedMultiplier * ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("SPEggHatchSpeedMultiplier", 10.0)
		    BabyMatureSpeedMultiplier = BabyMatureSpeedMultiplier * ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("SPBabyMatureSpeedMultiplier", 36.799)
		    BabyCuddleIntervalMultiplier = BabyCuddleIntervalMultiplier * ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("SPBabyCuddleIntervalMultiplier", 0.167)
		  End If
		  
		  If Self.mActiveEvent.IsEmpty = False Then
		    Var ActiveEvent As ArkSA.GameEvent = ArkSA.DataSource.Pool.Get(False).GetGameEventByArkCode(Self.mActiveEvent)
		    If (ActiveEvent Is Nil) = False Then
		      Var MatingIntervalMultiplierKey As ArkSA.ConfigOption = ArkSA.DataSource.Pool.Get(False).GetConfigOption(ArkSA.ConfigFileGame, ArkSA.HeaderShooterGame, "MatingIntervalMultiplier")
		      ...
		      Var EggHatchSpeedMultiplierKey As ArkSA.ConfigOption = ArkSA.DataSource.Pool.Get(False).GetConfigOption(ArkSA.ConfigFileGame, ArkSA.HeaderShooterGame, "EggHatchSpeedMultiplier")
		      ...
		      Var BabyMatureSpeedMultiplierKey As ArkSA.ConfigOption = ArkSA.DataSource.Pool.Get(False).GetConfigOption(ArkSA.ConfigFileGame, ArkSA.HeaderShooterGame, "BabyMatureSpeedMultiplier")
		      ...
		      Var BabyCuddleIntervalMultiplierKey As ArkSA.ConfigOption = ArkSA.DataSource.Pool.Get(False).GetConfigOption(ArkSA.ConfigFileGame, ArkSA.HeaderShooterGame, "BabyCuddleIntervalMultiplier")
	...
	#tag Method, Flags = &h21
		Private Sub UpdateStats()
		  ...
		  Var CuddlePeriod As Integer = ArkSA.DataSource.Pool.Get(False).GetIntegerVariable("Cuddle Period") * ImprintPeriodMultiplier
		  Var Creatures() As ArkSA.Creature = ArkSA.DataSource.Pool.Get(False).GetCreatures("", Self.Project.ContentPacks)
...
#tag Events ImprintPeriodPreviewField
	#tag Event
		Sub TextChanged()
		  ...
		  Var CuddlePeriod As Integer = Round(ArkSA.DataSource.Pool.Get(False).GetIntegerVariable("Cuddle Period") * Self.mBaselineRates.Value("BabyCuddleIntervalMultiplier").DoubleValue)
...
#tag Events ConfigToolbar
	...
	#tag Event
		Sub ItemPressed(Item As OmniBarItem, ItemRect As Rect)
		  ...
		  Case "AutoTuneButton"
		    Var Creatures() As ArkSA.Creature = ArkSA.DataSource.Pool.Get(False).GetCreatures("", Self.Project.ContentPacks)
		    Var Interval As Double = ArkSABreedingTunerDialog.Present(Self, Self.mBaselineRates, Self.Config(False).BabyMatureSpeedMultiplier, Self.Config(False).BabyImprintAmountMultiplier, Creatures)
		    If Interval > 0 Then
		  ...
		  Case "BaselineButton"
		    ...
		    Var Events() As ArkSA.GameEvent = ArkSA.DataSource.Pool.Get(False).GetGameEvents()
		    For Each GameEvent As ArkSA.GameEvent In Events
