#tag DesktopWindow
Begin BeaconDialog ArkSABreedingTunerDialog
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   False
   Composite       =   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub CheckCreatures(List As String)
		  ...
		  Else
		    ...
		    For I As Integer = 0 To Self.CreaturesList.RowCount - 1
		      Var ClassString As String = ArkSA.Creature(Self.CreaturesList.RowTagAt(I)).ClassString
	...
	#tag Method, Flags = &h21
		Private Sub Constructor(BaselineRates As Dictionary, MatureSpeedMultiplier As Double, ImprintAmountMultiplier As Double, Creatures() As ArkSA.Creature)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, BaselineRates As Dictionary, MatureSpeedMultiplier As Double, ImprintAmountMultiplier As Double, Creatures() As ArkSA.Creature) As Double
		  If Parent = Nil Then
		    Return 0
		  End If
		  
		  Var Win As New ArkSABreedingTunerDialog(BaselineRates, MatureSpeedMultiplier, ImprintAmountMultiplier, Creatures)
...
#tag Events ActionButton
	#tag Event
		Sub Pressed()
		  ...
		  Var OfficialCuddlePeriod As Integer = Round(ArkSA.DataSource.Pool.Get(False).GetIntegerVariable("Cuddle Period") * Self.mBaselineRates.Value("BabyCuddleIntervalMultiplier").DoubleValue)
		...
		Sub CellAction(row As Integer, column As Integer)
		  ...
		  For I As Integer = 0 To Self.CreaturesList.RowCount - 1
		    If Self.CreaturesList.CellCheckBoxValueAt(I, Column) Then
		      Classes.Add(ArkSA.Creature(Self.CreaturesList.RowTagAt(I)).ClassString)
		    End If
		  Next
...
#tag Events MajorCreaturesButton
	#tag Event
		Sub Pressed()
		  Self.CheckCreatures(ArkSA.DataSource.Pool.Get(False).GetStringVariable("Major Imprint Creatures"))
		End Sub
	#tag EndEvent
#tag EndEvents
