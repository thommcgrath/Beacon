#tag DesktopWindow
Begin ArkSAConfigEditor ArkSASpoilTimersEditor
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  Self.mScorchedSpoilMultiplier = ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("Scorched Spoil Multiplier", 0.9)
		  Self.mDecayPeriods = New Dictionary
		  Self.mSpoilMultipliers = New Dictionary
		  Self.mSpoilTimes = New Dictionary
		  
		  Try
		    Var JSON As String = ArkSA.DataSource.Pool.Get(False).GetStringVariable("Decay Periods")
		  ...
		  Try
		    Var JSON As String = ArkSA.DataSource.Pool.Get(False).GetStringVariable("Spoil Multipliers")
		    If JSON.IsEmpty = False Then
		  ...
		  Try
		    Var JSON As String = ArkSA.DataSource.Pool.Get(False).GetStringVariable("Spoil Times")
		    If JSON.IsEmpty = False Then
	...
	#tag Event
		Sub SetupUI()
		  Var Config As ArkSA.Configs.SpoilTimers = Self.Config(False)
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.Configs.SpoilTimers
		  Return ArkSA.Configs.SpoilTimers(Super.Config(ForWriting))
		End Function
	#tag EndMethod
	...
	#tag Event
		Sub TextChanged()
		  ...
		  If Self.Focus <> Self.CorpseDecomposePreviewField Then
		    Var Duration As Double = ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("Corpse Decompose Time", 900) * Value
	...
	#tag Event
		Sub TextChanged()
		  ...
		  If Self.Focus <> Self.ItemDecomposePreviewField Then
		    Var Duration As Double = ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("Item Decompose Time", 120) * Value
...
#tag Events CorpseDecomposePreviewField
	#tag Event
		Sub TextChanged()
		  ...
		  Var DefaultTime As Double = ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("Corpse Decompose Time", 900)
...
#tag Events ItemDecomposePreviewField
	#tag Event
		Sub TextChanged()
		  ...
		  Var Seconds As Integer = Interval.TotalSeconds
		  Var DefaultTime As Double = ArkSA.DataSource.Pool.Get(False).GetDoubleVariable("Item Decompose Time", 120)
		  Var Multiplier As Double = Seconds / DefaultTime
