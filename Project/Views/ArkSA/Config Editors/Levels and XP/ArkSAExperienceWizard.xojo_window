#tag DesktopWindow
Begin BeaconDialog ArkSAExperienceWizard
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   False
   Composite       =   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub mComputeThread_Run(Sender As Thread)
		  ...
		  Var XTimes As Dictionary = ArkSA.Configs.ExperienceCurves.PrecomputeCurveX(Curve, AdditionalLevels, 0.00001)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, StartingLevel As Integer, StartingXP As UInt64) As UInt64()
		  Var Win As New ArkSAExperienceWizard(StartingLevel, StartingXP)
	...
	#tag Method, Flags = &h21
		Private Sub UpdateList()
		  ...
		  Self.mValidXPAmount = (Self.mStartingXP + Self.mDesiredXP) < CType(ArkSA.Configs.ExperienceCurves.MaxSupportedXP, UInt64)
		  ...
		  End If
		    ...
		    Var Allowed As Boolean = EndingXP <= CType(ArkSA.Configs.ExperienceCurves.MaxSupportedXP, UInt64)
	...
	#tag Event
		Sub Pressed()
		  If Not Self.mValidXPAmount Then
		    Self.ShowAlert("XP total is too high", "Ark has a limit of " + ArkSA.Configs.ExperienceCurves.MaxSupportedXP.ToString(Locale.Current, "#,##0") + " total experience.")
		    Return
		  End If
