#tag DesktopWindow
Begin BeaconDialog ArkSAAdjustIngredientDialog
   Backdrop        =   0
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  Var EngramTags() As String = ArkSA.DataSource.Pool.Get(False).GetTags(Self.mProject.ContentPacks, ArkSA.CategoryEngrams)
	...
	#tag Method, Flags = &h21
		Private Function IngredientsCaption(Engrams() As ArkSA.Engram) As String
	...
	#tag Method, Flags = &h21
		Private Function RecipesCaption(Engrams() As ArkSA.Engram) As String
		  If Engrams Is Nil Or Engrams.Count = 0 Then
		    Return "No selection"
		  ElseIf Engrams.Count = 1 Then
		    Return Engrams(0).Label
	...
	#tag Property, Flags = &h21
		Private mReplacement As ArkSA.Engram
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mTargetIngredients() As ArkSA.Engram
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mTargetRecipes() As ArkSA.Engram
	#tag EndProperty
...
#tag Events TargetIngredientChooseButton
	#tag Event
		Sub Pressed()
		  ...
		  Var Engrams() As ArkSA.Engram = ArkSABlueprintSelectorDialog.Present(Self, "", Exclude, Self.mProject.ContentPacks, ArkSABlueprintSelectorDialog.SelectModes.ExplicitMultipleWithExcluded)
...
#tag Events ReplacementChooseButton
	#tag Event
		Sub Pressed()
		  Var Exclude() As ArkSA.Engram
		  Var Engrams() As ArkSA.Engram = ArkSABlueprintSelectorDialog.Present(Self, "", Exclude, Self.mProject.ContentPacks, ArkSABlueprintSelectorDialog.SelectModes.Single)
		...
		Sub Pressed()
		  ...
		  Case Self.TargetModeEdited
		    ...
		    If Config IsA ArkSA.Configs.CraftingCosts Then
		      Self.mTargetRecipes = ArkSA.Configs.CraftingCosts(Config).Engrams
		    End If
		  End Select
...
#tag Events ProcessorThread
	#tag Event
		Sub Run()
		  Var OriginalConfig As ArkSA.ConfigGroup = Self.mProject.ConfigGroup(ArkSA.Configs.NameCraftingCosts)
		  Var WorkingConfig As ArkSA.Configs.CraftingCosts
		  If OriginalConfig Is Nil Then
		    WorkingConfig = New ArkSA.Configs.CraftingCosts
		  Else
		    WorkingConfig = ArkSA.Configs.CraftingCosts(ArkSA.Configs.CloneInstance(OriginalConfig))
		  End If
		  
		  Var Engrams() As ArkSA.Engram = WorkingConfig.Engrams
		  ...
		  Var TargetRecipes() As ArkSA.Engram
		  If Self.mTargetRecipes.Count > 0 Then
		  ...
		  Var TargetIngredients() As ArkSA.Engram
		  If Self.mTargetIngredients.Count > 0 Then
		  ...
		  For Each Engram As ArkSA.Engram In Engrams
		    ...
		    Var Temp As ArkSA.CraftingCost = WorkingConfig.Cost(Engram)
		    Var Cost As ArkSA.MutableCraftingCost
		    If Temp Is Nil Then
		      Cost = New ArkSA.MutableCraftingCost(Engram, True)
		    Else
		      Cost = New ArkSA.MutableCraftingCost(Temp)
		    End If
		    ...
		    For IngredientIdx As Integer = Cost.LastIndex DownTo 0
		      ...
		      Var Replacement As ArkSA.Engram = If(Self.mReplacement Is Nil, Ingredient.Engram, Self.mReplacement)
		      ...
		      If Quantity = 0 And Self.mRemoveZeroQuantities = True Then
		        Cost.Remove(IngredientIdx)
		      Else
		        Cost.Ingredient(IngredientIdx) = New ArkSA.CraftingCostIngredient(Replacement, Quantity, Ingredient.RequireExact)
		      End If
