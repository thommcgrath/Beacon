#tag DesktopWindow
Begin ArkSAConfigEditor ArkSACraftingCostsEditor
...
#tag WindowCode
	...
	#tag Event
		Function ParsingFinished(Project As ArkSA.Project) As Boolean
		  If Project Is Nil Or Project.HasConfigGroup(ArkSA.Configs.NameCraftingCosts) = False Then
		    Return True
		  End If
		  
		  Var OtherConfig As ArkSA.Configs.CraftingCosts = ArkSA.Configs.CraftingCosts(Project.ConfigGroup(ArkSA.Configs.NameCraftingCosts))
		  If OtherConfig = Nil Or OtherConfig.Count = 0 Then
		    Return True
		  End If
		  
		  Var Config As ArkSA.Configs.CraftingCosts = Self.Config(True)
		  Var NewCosts() As ArkSA.CraftingCost
		  Var NewEngrams() As ArkSA.Engram = OtherConfig.Engrams
	...
	#tag Event
		Sub ShowIssue(Issue As Beacon.Issue)
		  ...
		  Try
		    Var Cost As ArkSA.CraftingCost = Issue.UserData
		    If Cost <> Nil Then
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.Configs.CraftingCosts
		  Return ArkSA.Configs.CraftingCosts(Super.Config(ForWriting))
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub CreateFibercraftServer()
		  Var Fiber As ArkSA.Engram = ArkSA.DataSource.Pool.Get(False).GetEngram("244bc843-2540-486e-af4a-8824500c0e56")
	...
	#tag Method, Flags = &h21
		Private Sub SetListWidth(NewSize As Integer)
		  ...
		  Var ListWidth As Integer = Min(Max(NewSize, Self.ListMinWidth), AvailableSpace - ArkCraftingCostEditor.MinimumWidth)
	...
	#tag Method, Flags = &h21
		Private Sub ShowAddEngram()
		  ...
		  Var NewEngrams() As ArkSA.Engram = ArkSABlueprintSelectorDialog.Present(Self, "Crafting", CurrentEngrams, Self.Project.ContentPacks, ArkSABlueprintSelectorDialog.SelectModes.ImpliedMultiple, WithDefaults)
		  ...
		  Var NewCosts() As ArkSA.CraftingCost
		  For Each Engram As ArkSA.Engram In NewEngrams
		    Var Cost As New ArkSA.CraftingCost(Engram, WithDefaults)
		    Config.Add(Cost)
		    NewCosts.Add(Cost)
		  Next
	...
	#tag Method, Flags = &h21
		Private Sub ShowDuplicateSelection()
		  ...
		  Var NewEngrams() As ArkSA.Engram = ArkSABlueprintSelectorDialog.Present(Self, "Crafting", CurrentEngrams, Self.Project.ContentPacks, ArkSABlueprintSelectorDialog.SelectModes.ExplicitMultiple)
		  If NewEngrams = Nil Or NewEngrams.LastIndex = -1 Then
		    Return
		  End If
		  
		  Var SourceCost As ArkSA.CraftingCost = Self.List.RowTagAt(Self.List.SelectedRowIndex)
		  Config = Self.Config(True)
		  
		  Var NewCosts() As ArkSA.CraftingCost
		  For Each Engram As ArkSA.Engram In NewEngrams
		    Var Cost As New ArkSA.MutableCraftingCost(SourceCost)
		    Cost.Engram = Engram
		    Config.Add(Cost)
		    NewCosts.Add(Cost)
		  Next
	...
	#tag Method, Flags = &h21
		Private Sub UpdateList(SelectItems() As ArkSA.CraftingCost)
		  Var ScrollPosition As Integer = Self.List.ScrollPosition
		  Self.List.SelectionChangeBlocked = True
		  
		  Var RecipeIds() As String
		  For Each Item As ArkSA.CraftingCost In SelectItems
		  ...
		  For Idx As Integer = Engrams.FirstIndex To Engrams.LastIndex
		    ...
		    Var Cost As ArkSA.CraftingCost = Config.Cost(Engrams(Idx))
	...
	#tag Method, Flags = &h21
		Private Sub UpdateList(SelectItem As ArkSA.CraftingCost)
		  Var Arr() As ArkSA.CraftingCost
	...
	#tag Constant, Name = kClipboardType, Type = String, Dynamic = False, Default = \"com.thezaz.beacon.ArkSA.craftingcost", Scope = Private
	#tag EndConstant
...
#tag Events List
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  Var Costs() As ArkSA.CraftingCost
		  ...
		  Var Config As ArkSA.Configs.CraftingCosts = Self.Config(True)
		  For Each Cost As ArkSA.CraftingCost In Costs
		    Config.Remove(Cost)
		  Next
		  Self.Modified = True
		  Self.UpdateList()
	...
	#tag Event
		Sub PerformCopy(Board As Clipboard)
		  ...
		  Var SelectedCosts() As ArkSA.CraftingCost
		  For I As Integer = 0 To Me.RowCount - 1
		    If Not Me.RowSelectedAt(I) Then
		      Continue
		    End If
		    
		    Var Cost As ArkSA.CraftingCost = Me.RowTagAt(I)
		  ...
		  If Contents.IsNull = False Then
		    Try
		      ...
		      For Each Dict As Dictionary In Dicts
		        Var Cost As ArkSA.CraftingCost = ArkSA.CraftingCost.ImportFromBeacon(Dict)
	...
	#tag Event
		Sub ContentsChanged()
		  If Self.List.SelectedRowCount = 1 Then
		    ...
		    Var Cost As ArkSA.CraftingCost = Me.Target
...
#tag Events AdjusterThread
	...
	#tag Event
		Sub Run()
		  Var OriginalConfig As ArkSA.Configs.CraftingCosts = Self.Config(False)
		  Var Engrams() As ArkSA.Engram = OriginalConfig.Engrams
		  Var Filter As New Dictionary
		  For Each Engram As ArkSA.Engram In Engrams
		    Filter.Value(Engram.EngramId) = True
		  Next
		  
		  Var EngramIds() As String = ArkSA.DataSource.Pool.Get(False).GetRecipeEngramIds(Self.Project.ContentPacks, Self.Project.MapMask)
		  ...
		  Var ReplacementConfig As New ArkSA.Configs.CraftingCosts
		  For Each Engram As ArkSA.Engram In Engrams
		    ...
		    Var Cost As ArkSA.MutableCraftingCost
		    Var ExistingCost As ArkSA.CraftingCost = OriginalConfig.Cost(Engram)
		    If (ExistingCost Is Nil) = False Then
		      Cost = New ArkSA.MutableCraftingCost(ExistingCost)
		    Else
		      Cost = New ArkSA.MutableCraftingCost(Engram, True)
		    End If
		    ...
		    For IngredientIdx As Integer = 0 To Cost.LastIndex
		      Var Ingredient As ArkSA.CraftingCostIngredient = Cost.Ingredient(IngredientIdx)
		      Var Quantity As Integer = Ceiling(Ingredient.Quantity * Self.mCostMultiplier)
		      Cost.Ingredient(IngredientIdx) = New ArkSA.CraftingCostIngredient(Ingredient.Reference, Quantity, Ingredient.RequireExact)
		    Next
