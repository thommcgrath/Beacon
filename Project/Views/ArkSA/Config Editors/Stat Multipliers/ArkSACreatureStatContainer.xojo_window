#tag DesktopWindow
Begin DesktopContainer ArkSACreatureStatContainer
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub UpdateFigures(Key As String, Config As ArkSA.Configs.StatMultipliers, Creature As ArkSA.Creature, OnlyStatIndex As Integer, IncludeFocusControl As Boolean, ForceAsPercentage As Boolean, MultiplyByBase As Boolean)
		  ...
		  For Each Stat As ArkSA.Stat In Stats
		    ...
		    Select Case Key
		    Case ArkSA.Creature.KeyWild
		      Multiplier = Config.WildPerLevelMultiplier(Stat)
		    Case ArkSA.Creature.KeyTamed
		      Multiplier = Config.TamedPerLevelMultiplier(Stat)
		    Case ArkSA.Creature.KeyAdd
		      Multiplier = Config.TamedAddMultiplier(Stat)
		    Case ArkSA.Creature.KeyAffinity
		      Multiplier = Config.TamedAffinityMultiplier(Stat)
		    End Select
		    
		    If Stat.IsPercentage Then
		      If Key = Creature.KeyAdd Then
		        PerLevel = 100 * Creature.StatAddValue(Stat)
		      Else
		        PerLevel = Creature.StatValue(Stat, Key) * (100 + (100 * Creature.StatAddValue(Stat)))
		      End If
		    End If
	...
	#tag Hook, Flags = &h0
		Event MultiplierUpdated(StatIndex As Integer, Value As Double)
	#tag EndHook
	...
	#tag ComputedProperty, Flags = &h0
		#tag Getter
			Get
			  Return Self.Group.Caption
			End Get
		#tag EndGetter
		#tag Setter
			Set
			  Self.Group.Caption = Value
			End Set
		#tag EndSetter
		Caption As String
