#tag DesktopWindow
Begin ArkSAConfigEditor ArkSALootDropsEditor
...
#tag WindowCode
	#tag Event
		Function ParsingFinished(Project As ArkSA.Project) As Boolean
		  If Project Is Nil Or Project.HasConfigGroup(ArkSA.Configs.NameLootDrops) = False Then
		    Return False
		  End If
		  
		  Var OtherConfig As ArkSA.Configs.LootDrops = ArkSA.Configs.LootDrops(Project.ConfigGroup(ArkSA.Configs.NameLootDrops))
	...
	#tag Method, Flags = &h21
		Private Sub BuildQuickDropMenu(Menu As DesktopMenuItem)
		  ...
		  Var Containers() As ArkSA.LootContainer = Data.GetLootContainers("", Self.Project.ContentPacks, Nil, Preferences.ShowExperimentalLootSources)
		  ...
		  Var AllContainers() As ArkSA.LootContainer = Data.GetLootContainers("", Self.Project.ContentPacks, Nil)
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.Configs.LootDrops
		  Return ArkSA.Configs.LootDrops(Super.Config(ForWriting))
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h0
		Function GoToChild(DropClass As String, ItemSetUUID As String = "", EntryUUID As String = "", EngramClass As String = "") As Boolean
		  For Idx As Integer = 0 To Self.List.LastRowIndex
		    Var Container As ArkSA.LootDropOverride = Self.List.RowTagAt(Idx)
	...
	#tag Method, Flags = &h21
		Private Sub UpdateContainerList(LootDropIds() As String)
		  ...
		  Var AllContainers() As ArkSA.LootContainer = ArkSA.DataSource.Pool.Get(False).GetLootContainers("", Self.Project.ContentPacks, Nil)
	...
	#tag Event
		Sub PerformEdit()
		  For I As Integer = 0 To Me.RowCount - 1
		    ...
		    Var Config As ArkSA.Configs.LootDrops = Self.Config(False)
		    If ArkSAAddLootDropDialog.Present(Self, Config, Self.Project.MapMask, Self.Project.ContentPacks, Me.RowTagAt(I)) Then
		      Call Self.Config(True) // Actually saves the config to the document
		      Self.UpdateContainerList()
		      Self.Modified = True
		    End If
