#tag DesktopWindow
Begin BeaconDialog ArkSAAddLootDropDialog
   BackColor       =   &cFFFFFF00
   Backdrop        =   0
   CloseButton     =   False
   Composite       =   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Sub BuildSourceList()
		  ...
		  Var AllowedLootContainers() As ArkSA.LootContainer = Providers.GetLootContainers(Self.FilterField.Text.MakeUTF8, Self.mContentPacks, Nil, Preferences.ShowExperimentalLootSources)
		  ...
		  For I As Integer = 0 To Self.SourceList.RowCount - 1
		    ...
		    Var Container As ArkSA.LootContainer = Self.SourceList.RowTagAt(I)
		  ...
		  For Each Container As ArkSA.LootContainer In AllowedLootContainers
		    ...
		    Else
		      ...
		      If Not MapLabels.HasKey(ComboMask) Then
		        MapLabels.Value(ComboMask) = ArkSA.Maps.ForMask(ComboMask).Label
		      End If
	...
	#tag Method, Flags = &h21
		Private Sub ChooseSelectedLootSources()
		  ...
		  For I As Integer = 0 To Self.SourceList.RowCount - 1
		    ...
		    Var Source As ArkSA.LootContainer = SourceList.RowTagAt(I)
	...
	#tag Method, Flags = &h21
		Private Sub Constructor(Config As ArkSA.Configs.LootDrops, Mask As UInt64, ContentPacks As Beacon.StringList, Source As ArkSA.LootDropOverride, ShowAsDuplicate As Boolean)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Config As ArkSA.Configs.LootDrops, Mask As UInt64, ContentPacks As Beacon.StringList, Source As ArkSA.LootDropOverride = Nil, ShowAsDuplicate As Boolean = False) As Boolean
		  If Parent Is Nil Or Config Is Nil Then
		    Return False
		  End If
		  Parent = Parent.TrueWindow
		  
		  Var Maps() As ArkSA.Map = ArkSA.Maps.ForMask(Mask)
		  ...
		  Var Win As New ArkSAAddLootDropDialog(Config, Mask, ContentPacks, Source, ShowAsDuplicate)
	...
	#tag Method, Flags = &h21
		Private Sub ShowCustomize()
		  Var BasedOn As ArkSA.LootDropOverride
		  If (Self.mSource Is Nil) = False Then
		    BasedOn = Self.mSource
		  Else
		    Var Blueprint As New ArkSA.MutableLootContainer(ArkSA.UnknownBlueprintPath("LootContainers", "BeaconTemplateItem"), "669ab057-f90b-4137-9a75-f8f912d5b6c8")
		    BasedOn = New ArkSA.LootDropOverride(Blueprint, False)
		  End If
		  ...
		  Var Templates() As ArkSA.LootTemplate = ArkSA.DataSource.Pool.Get(False).GetLootTemplates()
		  
		  Self.CustomizeTemplatesList.RemoveAllRows()
		  For Each Template As ArkSA.LootTemplate In Templates
		  ...
		  For I As Integer = 0 To Self.CustomizeTemplatesList.RowCount - 1
		    Var Template As ArkSA.LootTemplate = Self.CustomizeTemplatesList.RowTagAt(I)
		    For Each Set As ArkSA.LootItemSet In BasedOn
	...
	#tag Property, Flags = &h21
		Private mConfig As ArkSA.Configs.LootDrops
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mDestinations() As ArkSA.LootDropOverride
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mSource As ArkSA.LootDropOverride
	#tag EndProperty
...
#tag Events CustomizeActionButton
	#tag Event
		Sub Pressed()
		  ...
		  Var SourceSets() As ArkSA.LootItemSet
		  If (Self.mSource Is Nil) = False Then
		    For Each Set As ArkSA.LootItemSet In Self.mSource
		      If Set.TemplateUUID.IsEmpty Or AllowedTemplates.IndexOf(Set.TemplateUUID) > -1 Or ArkSA.DataSource.Pool.Get(False).GetLootTemplateByUUID(Set.TemplateUUID) Is Nil Then
		        SourceSets.Add(Set)
		      End If
		  ...
		  For Each Destination As ArkSA.LootDropOverride In Self.mDestinations
		    ...
		    // Add the clones
		    For Each Set As ArkSA.LootItemSet In SourceSets
		      Mutable.Add(Set)
		    Next
		    
		    // Add newly selected templates
		    For Each TemplateID As String In AdditionalTemplates
		      Var Template As ArkSA.LootTemplate = ArkSA.DataSource.Pool.Get(False).GetLootTemplateByUUID(TemplateID)
		      If Template Is Nil Then
		        Continue
		      End If
		      
		      Mutable.Add(ArkSA.LootItemSet.FromTemplate(Template, Mutable, Self.mMask, Self.mContentPacks))
		    Next
...
#tag Events SourceList
	...
	#tag Event
		Function RowComparison(row1 as Integer, row2 as Integer, column as Integer, ByRef result as Integer) As Boolean
		  ...
		  Var Source1 As ArkSA.LootContainer = Me.RowTagAt(Row1)
		  Var Source2 As ArkSA.LootContainer = Me.RowTagAt(Row2)
