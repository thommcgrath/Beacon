#tag DesktopWindow
Begin BeaconDialog ArkSAEngramControlEngramOverrideWizard
   Backdrop        =   0
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  ...
		  Else
		    ...
		    For Each Engram As ArkSA.Engram In Self.mEngrams
	...
	#tag Method, Flags = &h21
		Private Sub Constructor(Project As ArkSA.Project, Engrams() As ArkSA.Engram)
		  ...
		  For Idx As Integer = 0 To Engrams.LastIndex
		    If ArkSA.DataSource.Pool.Get(False).BlueprintIsCustom(Engrams(Idx)) = False And Engrams(Idx).ManualUnlock = False Then
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Project As ArkSA.Project, Engrams() As ArkSA.Engram) As ArkSA.Engram()
		  Var Edited() As ArkSA.Engram
		  If Parent = Nil Then
		    Return Edited
		  End If
		  
		  Var Win As New ArkSAEngramControlEngramOverrideWizard(Project, Engrams)
	...
	#tag Property, Flags = &h21
		Private mEditedEngrams() As ArkSA.Engram
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mEngrams() As ArkSA.Engram
	#tag EndProperty
	...
	#tag Event
		Sub Pressed()
		  ...
		  If Self.mEngrams.LastIndex = -1 Then
		    ...
		    End If
		    For Each Engram As ArkSA.Engram In ExistingEngrams
		      Engrams.Add(Engram)
		    Next
		  Else
