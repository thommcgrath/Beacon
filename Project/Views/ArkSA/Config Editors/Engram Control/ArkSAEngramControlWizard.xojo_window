#tag DesktopWindow
Begin BeaconDialog ArkSAEngramControlWizard
   Backdrop        =   0
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Project As ArkSA.Project) As Boolean
		  If Parent Is Nil Then
		    Return False
		  End If
		  
		  Var Win As New ArkSAEngramControlWizard(Project)
...
#tag Events WorkThread
	#tag Event
		Sub Run()
		  ...
		  If Self.mProject.HasConfigGroup(ArkSA.Configs.NameEngramControl) Then
		    Config = ArkSA.Configs.EngramControl(Self.mProject.ConfigGroup(ArkSA.Configs.NameEngramControl, False))
		  Else
		  ...
		  Var Engrams() As ArkSA.Engram = Beacon.Merge(Config.Engrams, ArkSA.DataSource.Pool.Get(False).GetEngramEntries("", Self.mProject.ContentPacks, Nil))
