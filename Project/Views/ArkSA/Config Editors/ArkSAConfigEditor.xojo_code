#tag Class
Protected Class ArkSAConfigEditor
...
Implements BeaconUI.ConfigEditorView
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As ArkSA.ConfigGroup
		  ...
		  If (Self.mConfigRef Is Nil) = False And (Self.mConfigRef.Value Is Nil) = False Then
		    Config = ArkSA.ConfigGroup(Self.mConfigRef.Value)
		  ...
		  Else
		    Config = ArkSA.Configs.CreateInstance(InternalName)
	...
	#tag Method, Flags = &h1
		Protected Sub Parse(GameUserSettingsIniContent As String, GameIniContent As String, Source As String)
		  ...
		  Var Parser As New ArkSA.ImportThread(Data, Self.mProject)
	...
	#tag Method, Flags = &h21
		Private Sub Parser_Finished(Sender As ArkSA.ImportThread, Project As ArkSA.Project)
		  ...
		  Var NewGroup As ArkSA.ConfigGroup = Project.ConfigGroup(InternalName)
		  ...
		  If Self.Project.HasConfigGroup(InternalName) And Self.Project.ConfigGroup(InternalName).SupportsMerging Then
		    ...
		    Var OriginalGroup As ArkSA.ConfigGroup = Self.Project.ConfigGroup(InternalName)
		    Var Editors(1) As ArkSA.ConfigGroup
		    Editors(0) = OriginalGroup
		    Editors(1) = NewGroup
		    Var ZeroHasPriority As Boolean = Choice = BeaconUI.ConfirmResponses.Alternate
		    Var Merged As ArkSA.ConfigGroup = ArkSA.Configs.Merge(Editors, ZeroHasPriority)
	...
	#tag Hook, Flags = &h0
		Event ParsingFinished(Project As ArkSA.Project) As Boolean
	#tag EndHook

	#tag Hook, Flags = &h0
		Event RunTool(Tool As ArkSA.ProjectTool)
	#tag EndHook
