#tag DesktopWindow
Begin BeaconDialog ArkSABlueprintEditorDialog
   Backdrop        =   0
   ...
   Begin DesktopPagePanel Pages
      ...
      Begin BeaconListbox CreatureStatsList
         ...
         _ScrollWidth    =   -1
      End
      Begin ArkSASpawnPointEditor SpawnPointEditor1
         AllowAutoDeactivate=   True
         AllowFocus      =   False
         AllowFocusRing  =   False
         AllowTabs       =   True
      ...
      Begin UITweaks.ResizedLabel LootIconLabel
         ...
         Width           =   132
      End
      Begin ArkSALootDropEditor DropEditor
         AllowAutoDeactivate=   True
         AllowFocus      =   False
         AllowFocusRing  =   False
         AllowTabs       =   True
...
#tag WindowCode
	#tag Event
		Sub Opening()
		  ...
		  If (Self.mOriginalBlueprint Is Nil) = False Then
		    ...
		    Self.MapSelector.SetWithMaps(ArkSA.Maps.ForMask(Self.mOriginalBlueprint.Availability))
		    If Self.mOriginalBlueprint.Availability = ArkSA.Maps.UniversalMask Then
		    ...
		    Select Case Self.mOriginalBlueprint
		    Case IsA ArkSA.Engram
		      Self.LoadBlueprint(ArkSA.Engram(Self.mOriginalBlueprint))
		    Case IsA ArkSA.Creature
		      Self.LoadBlueprint(ArkSA.Creature(Self.mOriginalBlueprint))
		    Case IsA ArkSA.SpawnPoint
		      Self.LoadBlueprint(ArkSA.SpawnPoint(Self.mOriginalBlueprint))
		    Case IsA ArkSA.LootContainer
		  ...
		  Else
		    ...
		    Self.NameField.Text = ""
		    Self.TagsField.Text = ""
		    Self.MapSelector.SetWithMaps(ArkSA.Maps.All)
		    Self.AllMapsRadio.Value = True
		  End If
	...
	#tag Method, Flags = &h21
		Private Sub Constructor(Blueprint As ArkSA.Blueprint)
	...
	#tag Method, Flags = &h21
		Private Sub LoadBlueprint(Creature As ArkSA.Creature)
		  ...
		  For Each Stat As ArkSA.Stat In Stats
		    Var Values As ArkSA.CreatureStatValue = Creature.StatValue(Stat)
	...
	#tag Method, Flags = &h21
		Private Sub LoadBlueprint(Engram As ArkSA.Engram)
	...
	#tag Method, Flags = &h21
		Private Sub LoadBlueprint(Container As ArkSA.LootContainer)
	...
	#tag Method, Flags = &h21
		Private Sub LoadBlueprint(Point As ArkSA.SpawnPoint)
		  Var Mutable As New ArkSA.MutableSpawnPointOverride(Point.MutableVersion, ArkSA.MutableSpawnPointOverride.ModeOverride)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, Blueprint As ArkSA.Blueprint) As ArkSA.Blueprint
		  ...
		  Var Win As New ArkSABlueprintEditorDialog(Blueprint)
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As DesktopWindow, ContentPackId As String, ContentPackName As String) As ArkSA.Blueprint
		  If Parent Is Nil Then
		    Return Nil
		  End If
		  
		  Var Win As New ArkSABlueprintEditorDialog(ContentPackId, ContentPackName)
		  Win.ShowModal(Parent)
	...
	#tag Method, Flags = &h21
		Private Sub PutIngredientInList(Ingredient As ArkSA.CraftingCostIngredient, RowIdx As Integer = -1)
	...
	#tag Method, Flags = &h21
		Private Function SampleProject() As ArkSA.Project
	...
	#tag Method, Flags = &h0
		Function Save() As Boolean
		  ...
		  If Self.AllMapsRadio.Value Then
		    Availability = ArkSA.Maps.UniversalMask
		  Else
		    Availability = ArkSA.Maps.MaskForMaps(Self.MapSelector.CheckedMaps)
		  End If
		  ...
		  If Self.mOriginalBlueprint Is Nil Then
		    ...
		    Case Self.IndexSpawnPoint
		      Blueprint = New ArkSA.MutableSpawnPoint(Path, BlueprintId)
		    Case Self.IndexLootContainer
		      Blueprint = New ArkSA.MutableLootContainer(Path, BlueprintId)
		    End Select
		  Else
		  ...
		  Case IsA ArkSA.MutableEngram
		    If Not Self.UpdateBlueprint(ArkSA.MutableEngram(Blueprint)) Then
		      Return False
		    End If
		  Case IsA ArkSA.MutableCreature
		    If Not Self.UpdateBlueprint(ArkSA.MutableCreature(Blueprint)) Then
		      Return False
		    End If
		  Case IsA ArkSA.MutableSpawnPoint
		    If Not Self.UpdateBlueprint(ArkSA.MutableSpawnPoint(Blueprint)) Then
		      Return False
		    End If
		  Case IsA ArkSA.MutableLootContainer
		    If Not Self.UpdateBlueprint(ArkSA.MutableLootContainer(Blueprint)) Then
		      Return False
		    End If
		  End Select
	...
	#tag Method, Flags = &h21
		Private Sub ShowStatInRow(Row As Integer, Values As ArkSA.CreatureStatValue)
		  ...
		  Self.CreatureStatsList.CellTextAt(Row, Self.CreatureStatColumnBase) = If(BaseValue <> ArkSA.Creature.MissingStatValue, BaseValue.PrettyText(True), "")
		  
		  Var WildValue As Double = Values.WildMultiplier
		  Self.CreatureStatsList.CellTextAt(Row, Self.CreatureStatColumnWild) = If(WildValue <> ArkSA.Creature.MissingStatValue, WildValue.PrettyText(True), "")
		  
		  Var TamedValue As Double = Values.TamedMultiplier
		  Self.CreatureStatsList.CellTextAt(Row, Self.CreatureStatColumnTamed) = If(TamedValue <> ArkSA.Creature.MissingStatValue, TamedValue.PrettyText(True), "")
		  
		  Var AddValue As Double = Values.AddMultiplier
		  Self.CreatureStatsList.CellTextAt(Row, Self.CreatureStatColumnAdd) = If(AddValue <> ArkSA.Creature.MissingStatValue, AddValue.PrettyText(True), "")
		  
		  Var AffinityValue As Double = Values.AffinityMultiplier
		  Self.CreatureStatsList.CellTextAt(Row, Self.CreatureStatColumnAffinity) = If(AffinityValue <> ArkSA.Creature.MissingStatValue, AffinityValue.PrettyText(True), "")
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Function UpdateBlueprint(Creature As ArkSA.MutableCreature) As Boolean
		  ...
		  For Row As Integer = 0 To Self.CreatureStatsList.LastRowIndex
		    Var Values As ArkSA.CreatureStatValue = Self.CreatureStatsList.RowTagAt(Row)
	...
	#tag Method, Flags = &h21
		Private Function UpdateBlueprint(Engram As ArkSA.MutableEngram) As Boolean
	...
	#tag Method, Flags = &h21
		Private Function UpdateBlueprint(Point As ArkSA.MutableSpawnPoint) As Boolean
		  Var Overrides() As ArkSA.SpawnPointOverride = Self.SpawnPointEditor1.Overrides
		  If Overrides.Count <> 1 Then
		    Return False
		  End If
		  
		  Var Source As ArkSA.SpawnPointOverride = Overrides(0)
		  
		  Point.Reset()
		  For Each Set As ArkSA.SpawnPointSet In Source
		    Point.AddSet(New ArkSA.SpawnPointSet(Set))
		  Next
		  
		  Var Refs() As ArkSA.BlueprintReference = Source.LimitedCreatureRefs
		  For Each Ref As ArkSA.BlueprintReference In Refs
	...
	#tag Property, Flags = &h21
		Private mModifiedBlueprint As ArkSA.Blueprint
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mOriginalBlueprint As ArkSA.Blueprint
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mSampleProject As ArkSA.Project
	#tag EndProperty
	...
	#tag Event
		Sub SelectionChanged(item As DesktopMenuItem)
		  ...
		  Case Self.IndexSpawnPoint
		    ...
		    TargetWidth = Max(TargetWidth, ArkSASpawnPointEditor.MinEditorWidth, ArkSALootDropEditor.MinEditorWidth)
		...
		Sub Pressed()
		  ...
		  For Each Engram As ArkSA.Engram In NewEngrams
		    If (Engram Is Nil) = False Then
		      Var Ingredient As New ArkSA.CraftingCostIngredient(Engram, 1, False)
...
#tag Events EngramCraftingCostList
		  ...
		  If Warn Then
		    For Row As Integer = 0 To Me.LastRowIndex
		      If Me.RowSelectedAt(Row) Then
		        Var Ingredient As ArkSA.CraftingCostIngredient = Me.RowTagAt(Row)
	...
	#tag Event
		Sub CellAction(row As Integer, column As Integer)
		  ...
		  Case 1
		    ...
		    Me.RowTagAt(Row) = New ArkSA.CraftingCostIngredient(Ingredient.Engram, Quantity, Ingredient.RequireExact)
		    Me.CellTextAt(Row, Column) = Quantity.ToString(Locale.Current, "#,##0")
		  Case 2
		    Me.RowTagAt(Row) = New ArkSA.CraftingCostIngredient(Ingredient.Engram, Ingredient.Quantity, Me.CellCheckBoxValueAt(Row, Column))
		  End Select
		End Sub
	#tag EndEvent
	#tag Event
	...
	#tag Event
		Function CanPaste(Board As Clipboard) As Boolean
		  Return Board.RawDataAvailable(ArkSA.CraftingCostIngredient.ClipboardType)
		End Function
	#tag EndEvent
	#tag Event
		  ...
		  Try
		    ...
		    For Each Dict As Dictionary In Dicts
		      ...
		      For Idx As Integer = 0 To Me.LastRowIndex
		        If ArkSA.CraftingCostIngredient(Me.RowTagAt(Idx)).Engram.ObjectId = Ingredient.Engram.ObjectId Then
...
#tag Events CreatureStatsList
	...
	#tag Event
		Function RowComparison(row1 as Integer, row2 as Integer, column as Integer, ByRef result as Integer) As Boolean
		  Var Values1 As ArkSA.CreatureStatValue = Me.RowTagAt(Row1)
		  Var Values2 As ArkSA.CreatureStatValue = Me.RowTagAt(Row2)
	...
	#tag Event
		Sub PerformEdit()
		  Var Values As ArkSA.CreatureStatValue = Me.RowTagAt(Me.SelectedRowIndex)
...
#tag Events SpawnPointEditor1
	#tag Event
		Sub Opening()
		  Var Overrides(0) As ArkSA.SpawnPointOverride
		  Overrides(0) = New ArkSA.SpawnPointOverride(New ArkSA.MutableSpawnPoint(ArkSA.UnknownBlueprintPath("SpawnPoints", "BlueprintEditor_C"), "8cdcd17e-b246-4973-a694-98e0dee33e25"), ArkSA.SpawnPointOverride.ModeOverride)
...
#tag Events LootIconColorMenu
	#tag Event
		Sub Opening()
		  Var LootColors() As Pair = ArkSA.LootColors
...
#tag Events LootIconMenu
	#tag Event
		Sub Opening()
		  Var Icons() As ArkSA.LootContainerIcon = ArkSA.DataSource.Pool.Get(False).GetLootContainerIcons
		  If Icons.Count = 0 Then
		    Me.AddRow("Generic", "3a1f5d12-0b50-4761-9f89-277492dc00e0")
		    Return
		  End If
		  For Each Icon As ArkSA.LootContainerIcon In Icons
