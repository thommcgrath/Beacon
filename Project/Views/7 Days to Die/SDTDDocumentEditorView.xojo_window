#tag DesktopWindow
Begin DocumentEditorView SDTDDocumentEditorView
   ...
   Begin SourceList ConfigList
      ...
      Width           =   230
   End
   Begin FadedSeparator SetPickerSeparator
      AllowAutoDeactivate=   True
      AllowFocus      =   False
      AllowFocusRing  =   True
      AllowTabs       =   False
   ...
   Begin DesktopPagePanel PagePanel1
      ...
      Begin OmniNoticeBar OmniNoticeBanner
         ...
         Width           =   569
      End
      Begin LogoFillCanvas ErrorCanvas
         AllowAutoDeactivate=   True
         AllowFocus      =   False
         AllowFocusRing  =   True
         AllowTabs       =   False
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Sub ActiveConfigSet(Assigns Value As Beacon.ConfigSet)
		  ...
		  If Value.IsBase = False And SDTD.Configs.SupportsConfigSets(ConfigName) = False Then
		    // If switching from base and on an editor that won't exist in the desired set, switch to something else
		    ConfigName = SDTD.Configs.NameGeneralSettings
		  End If
	...
	#tag Method, Flags = &h21
		Private Function ContinueWithoutExcludedConfigs() As Boolean
		  ...
		  If HumanNames.Count = 1 Then
		    ...
		    Explanation = Language.ReplacePlaceholders(Self.OmniWarningSingularExplanation, HumanNames(0), Language.GameName(SDTD.Identifier))
		  Else
		    Message = Self.OmniWarningPluralMessage
		    Explanation = Language.ReplacePlaceholders(Self.OmniWarningPluralExplanation, HumanNames.EnglishOxfordList(), Language.GameName(SDTD.Identifier))
		  End If
	...
	#tag Method, Flags = &h0
		Sub CurrentConfigName(Assigns Value As String)
		  ...
		  Var NewPanel As SDTDConfigEditor
		  ...
		  If (Self.CurrentPanel Is Nil) = False Then
		    ...
		    If Value.Length > 0 Then
		      RequiresPurchase = Not SDTD.Configs.ConfigUnlocked(Value, App.IdentityManager.CurrentIdentity)
		    End If
		    ...
		    If Embed Then
		      ...
		      If Self.CurrentPanel IsA SDTDServersEditor Then
		        AddHandler SDTDServersEditor(Self.CurrentPanel).ShouldDeployProfiles, WeakAddressOf ServersEditor_ShouldDeployProfiles
		      End If
	...
	#tag Method, Flags = &h21
		Private Sub DiscardConfigPanel(CacheKey As String)
		  ...
		  Var Panel As SDTDConfigEditor = Self.Panels.Value(CacheKey)
		  If Panel IsA SDTDServersEditor Then
		    RemoveHandler SDTDServersEditor(Panel).ShouldDeployProfiles, WeakAddressOf ServersEditor_ShouldDeployProfiles
		  End If
	...
	#tag Method, Flags = &h0
		Function Project() As SDTD.Project
		  Return SDTD.Project(Super.Project())
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub UpdateConfigList()
		  ...
		  Var Names() As String = SDTD.Configs.AllNames
		  ...
		  For I As Integer = 0 To Labels.LastIndex
		    ...
		    Var SupportsConfigSets As Boolean = SDTD.Configs.SupportsConfigSets(Tags(I))
	...
	#tag ComputedProperty, Flags = &h21
		#tag Getter
			Get
			  Return (Self.ProjectToolbar.Item("EmbeddedContentButton") Is Nil) = False
			End Get
		#tag EndGetter
		#tag Setter
			Set
			  If Value = Self.ShowEmbeddedContentButton Then
			    Return
			  End If
			  
			  If Value Then
			    Self.ProjectToolbar.Append(OmniBarItem.CreateSpace("EmbeddedContentSpaceA"))
			    Self.ProjectToolbar.Append(OmniBarItem.CreateSeparator("EmbeddedContentSeparator"))
			    Self.ProjectToolbar.Append(OmniBarItem.CreateSpace("EmbeddedContentSpaceB"))
			    
			    Var Button As OmniBarItem = OmniBarItem.CreateButton("EmbeddedContentButton", "Save Mod Content", IconToolbarGift, "This project has embedded mods that you can save.")
			    Button.AlwaysUseActiveColor = True
			    Button.ActiveColor = OmniBarItem.ActiveColors.Green
			    Self.ProjectToolbar.Append(Button)
			  Else
			    Self.ProjectToolbar.Remove("EmbeddedContentSpaceA")
			    Self.ProjectToolbar.Remove("EmbeddedContentSeparator")
			    Self.ProjectToolbar.Remove("EmbeddedContentSpaceB")
			    Self.ProjectToolbar.Remove("EmbeddedContentButton")
			  End If
			End Set
		#tag EndSetter
	...
	#tag Event
		Sub ItemPressed(Item As OmniBarItem, ItemRect As Rect)
		  ...
		  Case "ToolsButton"
		    ...
		    Var Tool As SDTD.ProjectTool = Choice.Tag
