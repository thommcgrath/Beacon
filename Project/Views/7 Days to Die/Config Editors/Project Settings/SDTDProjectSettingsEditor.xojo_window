#tag DesktopWindow
Begin SDTDConfigEditor SDTDProjectSettingsEditor
   ...
   Begin SwitchControl CompressedSwitch
      ...
      Width           =   40
   End
   Begin UITweaks.ResizedPopupMenu VersionMenu
      AllowAutoDeactivate=   True
      Bold            =   False
      Enabled         =   True
      FontName        =   "System"
...
#tag WindowCode
	#tag Event
		Sub SetupUI()
		  Var Versions() As Integer = SDTD.DataSource.Pool.Get(False).AllVersions
		  If Versions.IndexOf(Self.Project.GameVersion) = -1 Then
		    Versions.Add(Self.Project.GameVersion)
		  End If
		  Versions.Sort
		  
		  Self.VersionMenu.RemoveAllRows
		  For Idx As Integer = Versions.LastIndex DownTo 0
		    Var Version As Integer = Versions(Idx)
		    Self.VersionMenu.AddRow(SDTD.HumanVersion(Version), Version)
		  Next
	...
	#tag Constant, Name = HelpGameVersion, Type = String, Dynamic = False, Default = \"Choosing the version of the game you\'re playing help Beacon generate correct xml files.", Scope = Private
	#tag EndConstant
...
#tag Events VersionMenu
	#tag Event
		Sub SelectionChanged(item As DesktopMenuItem)
		  ...
		  Self.SettingUp = True
		  Self.Project.GameVersion = Me.SelectedRowTag
		  Self.Modified = True
		  Self.SettingUp = False
		End Sub
	#tag EndEvent
