#tag DesktopWindow
Begin DesktopContainer SDTDSettingsListContainer
...
#tag WindowCode
	...
	#tag Method, Flags = &h21
		Private Function CreateElement(Key As SDTD.ConfigOption, Left As Integer, Top As Integer, Width As Integer, Height As Integer) As SDTDSettingsListElement
		  Var Element As SDTDSettingsListElement
		  
		  Select Case Key.ValueType
		  Case SDTD.ConfigOption.ValueTypes.TypeBoolean
		    Element = New SDTDSettingsListBooleanElement(Key)
		  Case SDTD.ConfigOption.ValueTypes.TypeNumeric
		    Element = New SDTDSettingsListNumberElement(Key)
		  Case SDTD.ConfigOption.ValueTypes.TypeText
		    Element = New SDTDSettingsListStringElement(Key)
		  Else
		    Return Nil
		  End Select
		  ...
		  Var Config As SDTD.Configs.GeneralSettings = Self.Config(False)
	...
	#tag Method, Flags = &h21
		Private Function ElementForKey(Key As SDTD.ConfigOption) As SDTDSettingsListElement
		  For Each Element As SDTDSettingsListElement In Self.mElements
	...
	#tag Method, Flags = &h0
		Sub Filter(Assigns Value As String)
		  ...
		  For Each Key As SDTD.ConfigOption In AllKeys
		    ...
		    Var Members() As SDTD.ConfigOption
	...
	#tag Method, Flags = &h0
		Sub ForceReload()
		  ...
		  For Each Element As SDTDSettingsListElement In Self.mElements
		    ...
		    Var Key As SDTD.ConfigOption = Element.Key
		    Var Value As Variant = Config.Value(Key)
		    If IsNull(Value) Then
	...
	#tag Method, Flags = &h21
		Private Function IsCorrectElementClass(Element As SDTDSettingsListElement, Key As SDTD.ConfigOption) As Boolean
	...
	#tag Method, Flags = &h21
		Private Sub SettingChanged(Key As SDTD.ConfigOption, Value As Variant)
		  ...
		  Var Dependents() As Dictionary = Self.mDependencies.Value(KeyId)
		  For Each Dict As Dictionary In Dependents
		    Var TargetKey As SDTD.ConfigOption = Dict.Value("Target")
		    Var RequiredValue As Variant = Dict.Value("RequiredValue")
		    Var ShouldBeEnabled As Boolean = (Value = RequiredValue)
		    Var Element As SDTDSettingsListElement = Self.ElementForKey(TargetKey)
	...
	#tag Method, Flags = &h21
		Private Sub SetVisibilities(Force As Boolean = False)
		  ...
		  For GroupIdx As Integer = 0 To Self.mVisibleGroups.LastIndex
		    ...
		    Var Members() As SDTD.ConfigOption = Self.mVisibleKeys.Value(GroupName)
		    
		    Var Header As SDTDSettingsListHeader = Self.mGroupHeaders(GroupIdx)
		    ...
		    If HeaderBottom > 0 And HeaderTop < Self.Height Then
		      ...
		      If Header Is Nil Then
		        Header = New SDTDSettingsListHeader
		    ...
		    For Each Member As SDTD.ConfigOption In Members
		      Var Element As SDTDSettingsListElement = Self.mElements(ElementIdx)
	...
	#tag Method, Flags = &h21
		Private Function ShouldBeEnabled(Key As SDTD.ConfigOption) As Boolean
		  Var Requirements As Variant = Key.Constraint("other")
		  If IsNull(Requirements) Or (Requirements IsA Dictionary) = False Then
		    Return True
		  End If
		  
		  Var RequiredKey As SDTD.ConfigOption = SDTD.DataSource.Pool.Get(False).GetConfigOption(Dictionary(Requirements).Value("key").StringValue)
	...
	#tag Hook, Flags = &h0
		Event GetConfig(ForWriting As Boolean) As SDTD.Configs.GeneralSettings
	#tag EndHook
	...
	#tag Hook, Flags = &h0
		Event ValueChanged(Key As SDTD.ConfigOption, NewValue As Variant)
	#tag EndHook
	...
	#tag Property, Flags = &h21
		Private mElements() As SDTDSettingsListElement
	#tag EndProperty
	...
	#tag Property, Flags = &h21
		Private mGroupHeaders() As SDTDSettingsListHeader
	#tag EndProperty
	...
	#tag Property, Flags = &h0
		SettingChangeDelegate As SDTDGeneralSettingsEditor.SettingChangeDelegate
	#tag EndProperty
