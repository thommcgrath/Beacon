#tag Window
Begin ConfigEditor EngramControlConfigEditor
...
#tag WindowCode
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As BeaconConfigs.EngramControl
		  ...
		  If Self.mConfigRef <> Nil And Self.mConfigRef.Value <> Nil Then
		    Config = BeaconConfigs.EngramControl(Self.mConfigRef.Value)
		  ElseIf Document.HasConfigGroup(ConfigName) Then
		    Config = BeaconConfigs.EngramControl(Document.ConfigGroup(ConfigName))
	...
	#tag Method, Flags = &h21
		Private Sub SetupPointsList()
		  ...
		  Var OfficialLevels As Beacon.PlayerLevelData = LocalData.SharedInstance.OfficialPlayerLevelData
	...
	#tag Method, Flags = &h21
		Private Sub UpdateEngramsListStatus()
		  ...
		  Else
		    Self.EngramListStatus.Caption = Self.EngramList.SelectedRowCount.ToString + " of " + Language.NounWithQuantity(Self.EngramList.RowCount, "Engram Override", "Engram Overrides") + " Selected"
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub UpdatePointsListStatus()
		  ...
		  Else
		    Self.PointsListStatus.Caption = Self.PointsList.SelectedRowCount.ToString + " of " + Language.NounWithQuantity(Self.PointsList.RowCount, "Level", "Levels") + " Selected"
		  End If
		End Sub
	#tag EndMethod
	...
	#tag Constant, Name = kEngramsClipboardType, Type = String, Dynamic = False, Default = \"com.thezaz.beacon.engramoverride", Scope = Private
	#tag EndConstant

	#tag Constant, Name = kPointsClipboardType, Type = String, Dynamic = False, Default = \"com.thezaz.beacon.unlockpointoverride", Scope = Private
	#tag EndConstant
...
#tag Events EngramList
	...
	#tag Event
		Sub PerformClear(Warn As Boolean)
		  ...
		  Var Config As BeaconConfigs.EngramControl = Self.Config(True)
		  For Each Member As Beacon.Engram In Members
