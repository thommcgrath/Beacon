#tag Window
Begin BeaconDialog AdjustIngredientDialog
   Backdrop        =   0
...
#tag WindowCode
	...
	#tag Method, Flags = &h0
		Shared Function Present(Parent As Window, Document As Beacon.Document) As Boolean
		  Var Win As New AdjustIngredientDialog(Document)
	...
	#tag Property, Flags = &h21
		Private mReplacement As Beacon.Engram
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mTarget As Beacon.Engram
	#tag EndProperty
...
#tag Events TargetChooseButton
	#tag Event
		Sub Action()
		  ...
		  Var Engrams() As Beacon.Engram = EngramSelectorDialog.Present(Self, "", Exclude, Self.mDocument.Mods, EngramSelectorDialog.SelectModes.Single)
...
#tag Events ProcessorThread
	#tag Event
		Sub Run()
		  Var OriginalConfig As Beacon.ConfigGroup = Self.mDocument.ConfigGroup(BeaconConfigs.NameCraftingCosts)
		  Var WorkingConfig As BeaconConfigs.CraftingCosts
		  If OriginalConfig Is Nil Then
		    WorkingConfig = New BeaconConfigs.CraftingCosts
		  Else
		    WorkingConfig = BeaconConfigs.CraftingCosts(OriginalConfig.Clone(App.IdentityManager.CurrentIdentity, Self.mDocument))
		  End If
		  
		  Var Engrams() As Beacon.Engram = WorkingConfig.Engrams
		  Var Filter As New Dictionary
		  For Each Engram As Beacon.Engram In Engrams
		    Filter.Value(Engram.ObjectID) = True
		  Next
		  
		  Var ObjectIDs() As String = LocalData.SharedInstance.GetObjectIDsWithCraftingCosts(Self.mDocument.Mods, Beacon.Maps.UniversalMask)
		  ...
		  For Each Engram As Beacon.Engram In Engrams
		    ...
		    Var Cost As Beacon.CraftingCost = WorkingConfig.Cost(Engram)
		    If Cost Is Nil Then
		  ...
		  Self.mDocument.AddConfigGroup(WorkingConfig)
