#tag Window
Begin BeaconDialog SetupTransferrableElementDialog
   Backdrop        =   0
...
#tag WindowCode
	#tag Method, Flags = &h0
		Sub Constructor(Config As BeaconConfigs.CraftingCosts, Mods As Beacon.StringList)
		  Self.mConfig = Config
		  Self.mMods = Mods
		  Self.mIntermediate = Beacon.Data.GetEngramByID("0e795209-d421-432c-86a2-cd22bf5484a3")
		  Super.Constructor
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h0
		Shared Function Present(Parent As Window, Config As BeaconConfigs.CraftingCosts, Mods As Beacon.StringList) As Boolean
		  If Parent Is Nil Then
		    Return False
		  End If
		  
		  Var Win As New SetupTransferrableElementDialog(Config, Mods)
	...
	#tag Property, Flags = &h21
		Private mConfig As BeaconConfigs.CraftingCosts
	#tag EndProperty

	#tag Property, Flags = &h21
		Private mIntermediate As Beacon.Engram
	#tag EndProperty
...
#tag Events ChooseIntermediateButton
	#tag Event
		Sub Action()
		  Var Exclude() As Beacon.Engram
		  Var Engrams() As Beacon.Engram = EngramSelectorDialog.Present(Self, "", Exclude, Self.mMods, EngramSelectorDialog.SelectModes.Single)
...
#tag Events ActionButton
	#tag Event
		Sub Action()
		  Var Element As Beacon.Engram = Beacon.Data.GetEngramByID("904af0d9-ec92-447b-bcaa-d1a8bf1ab3cf")
		  
		  Var IntermediateCost As New Beacon.CraftingCost(Self.mIntermediate, False)
		  IntermediateCost.Append(Element, 1, True)
		  Self.mConfig.Add(IntermediateCost)
		  
		  If Self.CraftIntoShardsRadio.Value Then
		    Var ShardsCost As New Beacon.CraftingCost(Beacon.Data.GetEngramByID("c1ee52b8-e6c4-475d-96e9-b1b491b67063"), False)
		    ShardsCost.Append(Self.mIntermediate, 1, True)
		    Self.mConfig.Add(ShardsCost)
		    
		    Self.mConfig.Remove(Element)
		  Else
		    Var ElementCost As New Beacon.CraftingCost(Element, False)
		    ElementCost.Append(Self.mIntermediate, 1, True)
