#tag DesktopWindow
Begin DesktopContainer PalworldSettingsListContainer
...
#tag WindowCode
	...
	#tag Method, Flags = &h1
		Protected Function Config(ForWriting As Boolean) As Palworld.Configs.OtherSettings
		  Return RaiseEvent GetConfig(ForWriting)
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Function CreateElement(Key As Palworld.ConfigOption, Left As Integer, Top As Integer, Width As Integer, Height As Integer) As PalworldSettingsListElement
		  Var Element As PalworldSettingsListElement
		  
		  Select Case Key.ValueType
		  Case Palworld.ConfigOption.ValueTypes.TypeBoolean
		    Element = New PalworldSettingsListBooleanElement(Key)
		  Case Palworld.ConfigOption.ValueTypes.TypeNumeric
		    Element = New PalworldSettingsListNumberElement(Key)
		  Case Palworld.ConfigOption.ValueTypes.TypeText
		    Element = New PalworldSettingsListStringElement(Key)
		  Else
		    Return Nil
		  End Select
		  ...
		  Var Config As Palworld.Configs.OtherSettings = Self.Config(False)
	...
	#tag Method, Flags = &h21
		Private Function ElementForKey(Key As Palworld.ConfigOption) As PalworldSettingsListElement
		  For Each Element As PalworldSettingsListElement In Self.mElements
	...
	#tag Method, Flags = &h0
		Sub Filter(Assigns Value As String)
		  ...
		  For Each Key As Palworld.ConfigOption In AllKeys
		    If Palworld.Configs.OtherSettings.KeySupported(Key, ContentPacks) = False Then
		      Continue
		    End If
		    ...
		    Var Members() As Palworld.ConfigOption
	...
	#tag Method, Flags = &h0
		Sub ForceReload()
		  ...
		  For Each Element As PalworldSettingsListElement In Self.mElements
		    ...
		    Var Key As Palworld.ConfigOption = Element.Key
		    Var Value As Variant = Config.Value(Key)
		    If IsNull(Value) Then
	...
	#tag Method, Flags = &h21
		Private Function IsCorrectElementClass(Element As PalworldSettingsListElement, Key As Palworld.ConfigOption) As Boolean
		  Return (Element Is Nil) = False And (Key Is Nil) = False And Element.Key = Key
		End Function
	#tag EndMethod

	#tag Method, Flags = &h1
		Protected Function Project() As Palworld.Project
		  Return RaiseEvent GetProject()
		End Function
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub SettingChanged(Key As Palworld.ConfigOption, Value As Variant)
		  ...
		  For Each Dict As Dictionary In Dependents
		    Var TargetKey As Palworld.ConfigOption = Dict.Value("Target")
		    Var RequiredValue As Variant = Dict.Value("RequiredValue")
		    Var ShouldBeEnabled As Boolean = (Value = RequiredValue)
		    Var Element As PalworldSettingsListElement = Self.ElementForKey(TargetKey)
	...
	#tag Method, Flags = &h21
		Private Sub SetVisibilities(Force As Boolean = False)
		  ...
		  For GroupIdx As Integer = 0 To Self.mVisibleGroups.LastIndex
		    ...
		    Var Members() As Palworld.ConfigOption = Self.mVisibleKeys.Value(GroupName)
		    ...
		    For Each Member As Palworld.ConfigOption In Members
		      Var Element As PalworldSettingsListElement = Self.mElements(ElementIdx)
	...
	#tag Method, Flags = &h21
		Private Function ShouldBeEnabled(Key As Palworld.ConfigOption) As Boolean
		  Var Requirements As Variant = Key.Constraint("other")
		  If IsNull(Requirements) Or (Requirements IsA Dictionary) = False Then
		    Return True
		  End If
		  
		  Var RequiredKey As Palworld.ConfigOption = Palworld.DataSource.Pool.Get(False).GetConfigOption(Dictionary(Requirements).Value("key").StringValue)
	...
	#tag Hook, Flags = &h0
		Event GetConfig(ForWriting As Boolean) As Palworld.Configs.OtherSettings
	#tag EndHook

	#tag Hook, Flags = &h0
		Event GetProject() As Palworld.Project
	#tag EndHook

	#tag Hook, Flags = &h0
		Event ValueChanged(Key As Palworld.ConfigOption, NewValue As Variant)
	#tag EndHook
	...
	#tag Property, Flags = &h21
		Private mElements() As PalworldSettingsListElement
	#tag EndProperty
