#tag DesktopWindow
Begin PalworldConfigEditor PalworldServersEditor
...
#tag WindowCode
	#tag Event
		Sub Hidden()
		  Var Container As PalworldServerViewContainer = Self.CurrentView
	...
	#tag Method, Flags = &h21
		Private Sub FinishRefreshingDetails()
		  ...
		  For Each ViewId As Variant In ViewIds
		    Var Panel As PalworldServerViewContainer = Self.mViews.Value(ViewId)
		    Panel.Close
	...
	#tag Method, Flags = &h0
		Function InternalName() As String
		  Return Palworld.Configs.NameServers
		End Function
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub View_ContentsChanged(Sender As PalworldServerViewContainer)
		...
		#tag Setter
			Set
			  ...
			  If Self.mCurrentProfileID.IsEmpty = False Then
			    Var View As PalworldServerViewContainer = Self.mViews.Value(Self.mCurrentProfileID)
			    View.Visible = False
			    View.SwitchedFrom()
			  ...
			  Var View As PalworldServerViewContainer = Self.mViews.Value(Value)
			  View.SwitchedTo()
			  View.Visible = True
		...
		#tag Getter
			Get
			  If Self.mViews.HasKey(Self.mCurrentProfileID) Then
			    Return PalworldServerViewContainer(Self.mViews.Value(Self.mCurrentProfileID))
			  End If
			End Get
		#tag EndGetter
		CurrentView As PalworldServerViewContainer
	#tag EndComputedProperty
	...
	#tag Event
		Sub SelectionChanged()
		  ...
		  Case 1
		    ...
		    If Not Self.mViews.HasKey(ProfileID) Then
		      ...
		      Case Local.Identifier
		        View = New PalworldLocalServerView(Self.Project, Profile)
		      Case GameServerApp.Identifier
		        View = New PalworldGSAServerView(Self.Project, Profile)
		      Else
		        Self.CurrentProfileID = ""
		        Return
		      End Select
		  ...
		  Else
		    ...
		    If Not Self.mViews.HasKey(ProfileID) Then
		      Var View As New PalworldMultiServerView(Self.Project, Profiles)
		      View.EmbedWithin(Self, FadedSeparator1.Left + FadedSeparator1.Width, FadedSeparator1.Top, Self.Width - (FadedSeparator1.Left + FadedSeparator1.Width), FadedSeparator1.Height)
		      AddHandler View.ContentsChanged, WeakAddressOf View_ContentsChanged
		      Self.mViews.Value(ProfileID) = View
		    End If
		...
		Sub PerformClear(Warn As Boolean)
		  ...
		  For I As Integer = Me.LastRowIndex DownTo 0
		    If Me.RowSelectedAt(I) Then
		      Var Profile As Palworld.ServerProfile = Me.RowTagAt(I)
		      If Self.mViews.HasKey(Profile.ProfileID) Then
		        If Self.CurrentProfileID = Profile.ProfileID Then
		          Self.CurrentProfileID = ""
		        End If
		        
		        Var Panel As PalworldServerViewContainer = Self.mViews.Value(Profile.ProfileID)
		        Panel.Close
	...
	#tag Event
		Function ContextualMenuItemSelected(HitItem As DesktopMenuItem) As Boolean
		  ...
		  Case "Show Config Files"
		    Var LocalProfiles() As Palworld.ServerProfile = HitItem.Tag
		    For Idx As Integer = 0 To LocalProfiles.LastIndex
		      Var File As BookmarkedFolderItem = BookmarkedFolderItem.FromSaveInfo(LocalProfiles(Idx).SettingsIniPath)
	...
	#tag Event
		Sub Run()
		  ...
		  For Each Token As BeaconAPI.ProviderToken In Tokens
		    ...
		    Try
		      Profiles = Provider.ListServers(Config, Palworld.Identifier)
