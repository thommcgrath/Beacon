#tag DesktopWindow
Begin DocumentEditorView PalworldDocumentEditorView
   ...
   Begin SourceList ConfigList
      ...
      Width           =   230
   End
   Begin ConfigSetPicker SetPicker
      AllowAutoDeactivate=   True
      AllowFocus      =   False
      AllowFocusRing  =   True
      AllowTabs       =   False
...
#tag WindowCode
	...
	#tag Event
		Sub Hidden()
		  Var Panel As PalworldConfigEditor = Self.CurrentPanel
		  ...
		  If (Self.Project Is Nil) = False Then
		    ...
		    If LastConfigName.IsEmpty Or LastConfigSet Is Nil Then
		      If Self.URL.Type.OneOf(Beacon.ProjectURL.TypeWeb, Beacon.ProjectURL.TypeCommunity) Then
		        LastConfigName = Palworld.Configs.NameProjectSettings
		      Else
		        LastConfigName = Palworld.Configs.NameGeneralSettings
		      End If
	...
	#tag Method, Flags = &h21
		Private Sub BeginImport(ForDeployment As Boolean)
		  ...
		  Var ImportView As New PalworldImportView
	...
	#tag Method, Flags = &h0
		Sub CurrentConfigName(Assigns Value As String)
		  ...
		  If (Value = Palworld.Configs.NameAccounts Or Value = Palworld.Configs.NameServers Or Value = Palworld.Configs.NameProjectSettings) And Self.ActiveConfigSet.IsBase = False Then
		    Self.ActiveConfigSet = Beacon.ConfigSet.BaseConfigSet
		  End If
		  
		  Super.CurrentConfigName = Value
		  
		  Var NewPanel As PalworldConfigEditor
		  ...
		  If Value.IsEmpty = False Then
		    ...
		    Else
		      Select Case Value
		      Case Palworld.Configs.NameServers
		        NewPanel = New PalworldServersEditor(Self.Project)
		      Case Palworld.Configs.NameAccounts
		        NewPanel = New PalworldAccountsEditor(Self.Project)
		      Case Palworld.Configs.NameProjectSettings
		        NewPanel = New PalworldProjectSettingsEditor(Self.Project)
		      Case Palworld.Configs.NameCustomConfig
		        NewPanel = New PalworldCustomConfigEditor(Self.Project)
		      Case Palworld.Configs.NameGeneralSettings
		        NewPanel = New PalworldGeneralSettingsEditor(Self.Project)
		      End Select
		  ...
		  If (Self.CurrentPanel Is Nil) = False Then
		    ...
		    If Value.Length > 0 Then
		      RequiresPurchase = Not Palworld.Configs.ConfigUnlocked(Value, App.IdentityManager.CurrentIdentity)
		    End If
		    ...
		    If Embed Then
		      ...
		      If Self.CurrentPanel IsA PalworldServersEditor Then
		        AddHandler PalworldServersEditor(Self.CurrentPanel).ShouldDeployProfiles, WeakAddressOf ServersEditor_ShouldDeployProfiles
		      End If
	...
	#tag Method, Flags = &h21
		Private Sub DiscardConfigPanel(CacheKey As String)
		  ...
		  Var Panel As PalworldConfigEditor = Self.Panels.Value(CacheKey)
		  If Panel IsA PalworldServersEditor Then
		    RemoveHandler PalworldServersEditor(Panel).ShouldDeployProfiles, WeakAddressOf ServersEditor_ShouldDeployProfiles
		  End If
	...
	#tag Method, Flags = &h21
		Private Sub MergeCallback()
		  ...
		  For Each Key As Variant In Keys
		    Var Panel As PalworldConfigEditor = Self.Panels.Value(Key)
		    If Panel <> Nil Then
	...
	#tag Method, Flags = &h21
		Private Sub mValidator_ValidationComplete_Export(Sender As Beacon.ProjectValidator, Results As Beacon.ProjectValidationResults, UserData As Variant)
		  ...
		  PalworldExportWindow.Present(Self, Self.Project, Keyboard.AsyncOptionKey And Keyboard.AsyncShiftKey)
		End Sub
	#tag EndMethod
	...
	#tag Method, Flags = &h21
		Private Sub Panel_ContentsChanged(Sender As PalworldConfigEditor)
		  #Pragma Unused Sender
	...
	#tag Method, Flags = &h21
		Private Sub RunTool(ToolId As String)
		  #Pragma Unused ToolId
		  
		  // No project-level tools
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub ServersEditor_ShouldDeployProfiles(Sender As PalworldServersEditor, SelectedProfiles() As Beacon.ServerProfile)
		  #Pragma Unused Sender
	...
	#tag Method, Flags = &h21
		Private Sub UpdateConfigList()
		  ...
		  If IsBase Then
		    Var PsuedoTags() As String = Array(Palworld.Configs.NameProjectSettings, Palworld.Configs.NameAccounts, Palworld.Configs.NameServers)
		    // Show everything
		  ...
		  Var Names() As String = Palworld.Configs.AllNames
		  ...
		  For I As Integer = 0 To Labels.LastIndex
		    ...
		    Var SupportsConfigSets As Boolean = Palworld.Configs.SupportsConfigSets(Tags(I))
	...
	#tag Property, Flags = &h21
		Private CurrentPanel As PalworldConfigEditor
	#tag EndProperty
	...
	#tag Constant, Name = kConfigGroupClipboardType, Type = String, Dynamic = False, Default = \"com.thezaz.beacon.palworld.configgroup", Scope = Private
	#tag EndConstant
		...
		Sub ContextualClick(MouseX As Integer, MouseY As Integer, ItemIndex As Integer, ItemRect As Rect)
		  ...
		  Case PasteTag
		    Try
		      ...
		      Var EncryptedSaveData As Dictionary = Nil // The encrypted data will be inside NewConfigData. Nil tells the logic to get the data there.
		      Var NewConfig As Palworld.ConfigGroup = Palworld.Configs.CreateInstance(NewConfigName, NewConfigData, EncryptedSaveData)
...
#tag Events OmniBar1
	#tag Event
		Sub Opening()
		  Me.Append(OmniBarItem.CreateTitle("GameName", Palworld.FullName))
		...
		Sub ItemPressed(Item As OmniBarItem, ItemRect As Rect)
		  ...
		  Case "ToolsButton"
		    ...
		    For Each Tool As Palworld.ProjectTool In Tools
		      ...
		      Var Siblings() As Palworld.ProjectTool
		    ...
		    For Each GroupName As String In GroupNames
		      ...
		      Var GroupTools() As Palworld.ProjectTool = GroupedTools.Value(GroupName)
		      Var ToolNames() As String
		      For Each Tool As Palworld.ProjectTool In GroupTools
