...
$platform = $_GET['platform'] ?? 'any';
$show_latest = ($original_version > $current_version); // this means show the latest
$show_beacon2 = ($current_version >= 20000000 || $original_version >= 20000000);
$pages = [];
$page_direction = 'left';
...
if ($original_version < 1) {
	...
	if (($show_latest && $show_beacon2 === false) || ($current_version >= 10600000 && $original_version < 10600000)) {
		if ($platform === 'windows') {
	...
	if (($show_latest && $show_beacon2 === true) || ($current_version >= 20000000 && $original_version < 20000000)) {
		StartPage('Improved Project Sharing'); ?>
		<div class="duo duo-<?php echo $page_direction; ?>">
			<div class="duo-image"><img src="beacon200-project-sharing.png" width="298" height="186" alt=""></div>
			<div class="duo-text">
				<h1>Improved Project Sharing</h1>
				<p>Sharing cloud projects with other users now allows two additional user roles. Admins can add or remove non-Admin users, and Guests cannot save or generate configs for a project. Also, sharing changes no longer requires saving the project when finished.</p>
			</div>
		</div><?php
		EndPage();

		StartPage('Mod Content In Projects'); ?>
		<div class="duo duo-<?php echo $page_direction; ?>">
			<div class="duo-image"><img src="beacon200-project-mods.png" width="298" height="186" alt=""></div>
			<div class="duo-text">
				<h1>Mod Content In Projects</h1>
				<p>Enabled custom mods are automatically embedded into projects so the projects will still work correctly for users who do not have the mod info. As an added bonus, embedded mod content can be imported for later reuse.</p>
			</div>
		</div><?php
		EndPage();

		StartPage('Community Mods'); ?>
		<div class="duo duo-<?php echo $page_direction; ?>">
			<div class="duo-image"><img src="beacon200-community-mods.png" width="298" height="186" alt=""></div>
			<div class="duo-text">
				<h1>Community Mods</h1>
				<p>The Mod Discovery feature now automatically uploads its results to the Community tab in the Mods section of Beacon. Adding a community mod to your Beacon account is now requires just one click.</p>
			</div>
		</div><?php
		EndPage();
	}
