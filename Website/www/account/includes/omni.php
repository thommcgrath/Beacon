...
function ShowGiftCodes() {
	...
	while (!$results->EOF()) {
		...
		echo '<tr><td>' . htmlentities($code) . '<div class="row-details"><span class="detail">' . ($redeemed ? 'Redeemed' : BeaconCommon::AbsoluteURL('/redeem/' . htmlentities($code))) . '<span></div></td><td class="low-priority">' . ($redeemed ? 'Redeemed' : '&nbsp;') . '</td><td class="low-priority">' . ($redeemed ? '&nbsp;' : BeaconCommon::AbsoluteURL('/redeem/' . htmlentities($code))) . '</td>';
		$results->MoveNext();
	}
	echo '</table>';
}
