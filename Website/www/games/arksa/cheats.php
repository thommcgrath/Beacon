...
use BeaconAPI\v4\ArkSA\Blueprint;
...
$results = $database->Query("SELECT MAX(last_update) FROM arksa.objects WHERE min_version <= $1;", $build);
...
if (is_null($pack)) {
	...
	$title = 'Ark: Survival Ascended Spawn Codes';
	$baseUrl = '/Games/ArkSA/Cheats';
} else {
	$filters['contentPackId'] = $pack->ContentPackId();
	$title = 'Spawn codes for ' . $pack->Name();
	$includeModNames = false;
	$baseUrl = '/Games/ArkSA/Mods/' . urlencode($pack->MarketplaceId()) . '/Cheats';
...
?><table id="spawntable" class="generic">
	...
	if (count($engramIds) > 0) {
		$gfiRows = $database->Query("SELECT object_id, gfi FROM arksa.engrams WHERE object_id = ANY($1) AND gfi IS NOT NULL;", '{' . implode(',', $engramIds) . '}');
		while (!$gfiRows->EOF()) {
