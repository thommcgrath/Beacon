...
use BeaconAPI\v4\ArkSA\{Blueprint, Creature, Engram, GenericObject, LootDrop, Map, SpawnPoint};
...
if (count($contentPackNames) > 1) {
	...
	BeaconTemplate::SetCanonicalPath('/Games/ArkSA/' . urlencode($classString));
	...
	foreach ($contentPackNames as $marketplaceId => $name) {
		echo '<li><a href="/Games/ArkSA/Mods/' . htmlentities(urlencode($marketplaceId)) . '/' . htmlentities(urlencode($classString)) . '">' . htmlentities($name) . '</a>';
	}
	echo '</ul>';
	exit;
}

BeaconTemplate::SetCanonicalPath('/Games/ArkSA/Mods/' . $objects[0]->ContentPackMarketplaceId() . '/' . urlencode($objects[0]->ClassString()));
...
foreach ($objects as $object) {
	...
	if (count($tags) > 0) {
		...
		foreach ($tags as $tag) {
			...
			$links[] = '[' . $tagInfo['human'] . '](/Games/ArkSA/Tags/' . urlencode($tagInfo['url']) . '/' . urlencode($objectGroup) . ')';
		}
...
function MarkdownLinkToObject(GenericObject $obj) {
	return '[' . $obj->Label() . '](/Games/ArkSA/Mods/' . urlencode($obj->ContentPackMarketplaceId()) . '/' . urlencode($obj->ClassString()) . ')';
}
