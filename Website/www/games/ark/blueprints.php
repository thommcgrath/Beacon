...
use BeaconAPI\v4\Ark\{Blueprint, BlueprintGroup};
	...
	<div class="page-panel-page page-panel-visible" page="<?php echo htmlentities($currentGroup->ApiVersion()); ?>">
		...
		if ($pageContents['totalResults'] === 0) {
			...
			foreach ($pageContents['results'] as $result) {
				$modLink = '<a href="' . htmlentities('/Games/Ark/Mods/' . urlencode($result->ContentPackMarketplaceId())) . '">' . htmlentities($result->ContentPackName()) . '</a>';
				$modDetailLink = '';
				if (is_null($pack)) {
