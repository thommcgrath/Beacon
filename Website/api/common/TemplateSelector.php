...
class TemplateSelector implements \JsonSerializable {
	protected $object_id;
	protected $game_id;
	protected $label;
	...
	public static function GetAll(int $min_version = -1, \DateTime $updated_since = null) {
		...
		$sql = 'SELECT object_id, label, min_version, game_id, language, code FROM public.template_selectors';
		if (count($clauses) > 0) {
	...
	protected static function FromRow(\BeaconRecordSet $row) {
		...
		$obj->language = $row->Field('language');
		$obj->code = $row->Field('code');
		return $obj;
	}
}
