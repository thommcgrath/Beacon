...
class Engram extends \Ark\Blueprint {
	...
	protected static function SQLColumns() {
		...
		$columns[] = 'stack_size';
		$columns[] = 'item_id';
		$columns[] = '(SELECT array_to_json(array_agg(row_to_json(recipe_template))) FROM (SELECT ingredients.object_id, quantity, exact FROM ark.crafting_costs INNER JOIN ark.engrams AS ingredients ON (ark.crafting_costs.ingredient_id = ingredients.object_id) WHERE engram_id = ark.engrams.object_id) AS recipe_template) AS recipe';
		return $columns;
	}
