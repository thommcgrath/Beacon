...
class ApplicationAuthFlow extends DatabaseObject {
	...
	public static function Create(Application $app, array $scopes, string $callback, string $state, ?string $codeVerifierHash, ?string $codeVerifierMethod, ?string $publicKey): static {
		...
		$database->Query("INSERT INTO public.application_auth_flows (flow_id, application_id, scopes, callback, state, verifier_hash, verifier_hash_algorithm, public_key) VALUES ($1, $2, $3, $4, $5, $6, $7, $8);", $flowId, $app->ApplicationId(), $scopesString, $callback, $state, $codeVerifierHash, $codeVerifierMethod, $publicKey);
		$database->Commit();
	...
	public static function Redeem(string $applicationId, ?string $applicationSecret, string $redirectUri, string $code, string $codeVerifier): Session {
		...
		$flow = $flows[0];
		if ($flow->expired) {
			throw new Exception('Authorization flow has expired.');
		}
