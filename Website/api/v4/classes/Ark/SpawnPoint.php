...
class SpawnPoint extends MutableBlueprint {
	...
	protected function SaveSets(BeaconDatabase $database) {
		...
		if (is_null($this->sets) === false) {
			foreach ($this->sets as $set) {
				...
				$database->Upsert('ark.spawn_point_sets', $columns, ['spawn_point_set_id']);
		...
		if (count($setsList) > 0) {
			$database->Query('DELETE FROM ark.spawn_point_sets WHERE spawn_point_id = $1 AND spawn_point_set_id NOT IN (\'' . implode('\', \'', $setsList) . '\');', $spawnPointId);
		} else {
			$database->Query('DELETE FROM ark.spawn_point_sets WHERE spawn_point_id = $1;', $spawnPointId);
		}
	}
	...
	protected function SaveSetEntries(BeaconDatabase $database, string $spawnPointSetId, ?array $entries) {
		...
		if (count($keepEntries) > 0) {
			$database->Query('DELETE FROM ark.spawn_point_set_entries WHERE spawn_point_set_id = $1 AND spawn_point_set_entry_id NOT IN (\'' . implode('\', \'', $keepEntries) . '\');', $spawnPointSetId);
		} else {
			$database->Query('DELETE FROM ark.spawn_point_set_entries WHERE spawn_point_set_id = $1;', $spawnPointSetId);
		}
	}
	...
	protected function SaveSetEntryLevels(BeaconDatabase $database, string $spawnPointSetEntryId, ?array $levels) {
		...
		if (count($keepLevels) > 0) {
			$database->Query('DELETE FROM ark.spawn_point_set_entry_levels WHERE spawn_point_set_entry_id = $1 AND spawn_point_set_entry_level_id NOT IN (\'' . implode('\', \'', $keepLevels) . '\');', $spawnPointSetEntryId);
		} else {
			$database->Query('DELETE FROM ark.spawn_point_set_entry_levels WHERE spawn_point_set_entry_id = $1;', $spawnPointSetEntryId);
		}
	}
	...
	protected function SaveSetReplacements(BeaconDatabase $database, string $spawnPointSetId, ?array $replacements) {
		...
		if (count($keepReplacements) > 0) {
			$database->Query('DELETE FROM ark.spawn_point_set_replacements WHERE spawn_point_set_id = $1 AND spawn_point_set_replacement_id NOT IN (\'' . implode('\', \'', $keepReplacements) . '\');', $spawnPointSetId);
		} else {
			$database->Query('DELETE FROM ark.spawn_point_set_replacements WHERE spawn_point_set_id = $1;', $spawnPointSetId);
		}
	}
	...
	protected function SaveLimits(BeaconDatabase $database) {
		...
		if (is_null($this->limits) === false) {
			foreach ($this->limits as $limit) {
				...
				$database->Query('INSERT INTO ark.spawn_point_limits (spawn_point_id, creature_id, max_percentage) VALUES ($1, $2, $3) ON CONFLICT (spawn_point_id, creature_id) DO UPDATE SET max_percentage = $3 WHERE spawn_point_limits.max_percentage IS DISTINCT FROM $3;', $spawnPointId, $creatureId, $percentage);
			}
		}
		if (count($creatureList) > 0) {
			$database->Query('DELETE FROM ark.spawn_point_limits WHERE spawn_point_id = $1 AND creature_id NOT IN (\'' . implode('\', \'', $creatureList) . '\');', $spawnPointId);
		} else {
			$database->Query('DELETE FROM ark.spawn_point_limits WHERE spawn_point_id = $1;', $spawnPointId);
		}
	}
}
