...
use BeaconAPI\v4\{Core, DatabaseSchema, DatabaseSearchParameters};
use BeaconRecordSet;
...
class ConfigLine extends GenericObject {
	...
	protected function __construct(BeaconRecordSet $row) {
		...
		$this->native_editor_version = $row->Field('native_editor_version');
		$this->file = $row->Field('file');
		$this->header = $row->Field('header');
		$this->key = $row->Field('key');
		$this->value_type = $row->Field('value_type');
		$this->max_allowed = is_null($row->Field('max_allowed')) ? null : intval($row->Field('max_allowed'));
		$this->description = trim($row->Field('description'));
		$this->default_value = $row->Field('default_value');
		$this->nitrado_path = $row->Field('nitrado_path');
		$this->nitrado_format = $row->Field('nitrado_format');
		$this->nitrado_deploy_style = $row->Field('nitrado_deploy_style');
		$this->ui_group = $row->Field('ui_group');
		$this->constraints = is_null($row->Field('constraints')) ? null : json_decode($row->Field('constraints'), true);
		$this->custom_sort = $row->Field('custom_sort');
		$this->gsa_placeholder = $row->Field('gsa_placeholder');
	}
	...
	protected static function BuildSearchParameters(DatabaseSearchParameters $parameters, array $filters): void {
		...
		$table = $schema->table();
		$parameters->AddFromFilter($table, $filters, 'file');
		$parameters->AddFromFilter($table, $filters, 'header');
		$parameters->AddFromFilter($table, $filters, 'key');
	}
