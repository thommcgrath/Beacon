	...
	protected function SaveChildObjects(BeaconDatabase $database): void {
		...
		if (is_null($this->stats) === false) {
			foreach ($this->stats as $stat) {
				...
				$validStats[] = $index;
				$database->Query('INSERT INTO ark.creature_stats (creature_id, stat_index, base_value, per_level_wild_multiplier, per_level_tamed_multiplier, add_multiplier, affinity_multiplier) VALUES ($1, $2, $3, $4, $5, $6, $7) ON CONFLICT (creature_id, stat_index) DO UPDATE SET base_value = $3, per_level_wild_multiplier = $4, per_level_tamed_multiplier = $5, add_multiplier = $6, affinity_multiplier = $7 WHERE creature_stats.base_value IS DISTINCT FROM $3 OR creature_stats.per_level_wild_multiplier IS DISTINCT FROM $4 OR creature_stats.per_level_tamed_multiplier IS DISTINCT FROM $5 OR creature_stats.add_multiplier IS DISTINCT FROM $7 OR creature_stats.affinity_multiplier IS DISTINCT FROM $7;', $this->objectId, $index, $stat['base_value'], $stat['per_level_wild_multiplier'], $stat['per_level_tamed_multiplier'], $stat['add_multiplier'], $stat['affinity_multiplier']);
			}
		}

		$database->Query('DELETE FROM ark.creature_stats WHERE creature_id = $1 AND NOT (stat_index = ANY($2));', $this->objectId, '{' . implode(',', $validStats) . '}');
	}
}
