...
class BeaconSpawnPoint extends BeaconBlueprint {
	...
	protected static function SQLColumns() {
		...
		$columns[] = '(SELECT array_to_json(array_agg(row_to_json(sets_template))) FROM (SELECT label, spawn_set_id AS group_id, weight, array_to_json(ARRAY(SELECT creatures.path FROM (SELECT DISTINCT spawn_point_set_entries.creature_id FROM spawn_point_set_entries WHERE spawn_point_set_entries.spawn_point_set_id = spawn_point_sets.spawn_set_id) AS entry_creatures INNER JOIN creatures ON (entry_creatures.creature_id = creatures.object_id))) AS creatures FROM spawn_point_sets WHERE spawn_point_sets.spawn_point_id = spawn_points.object_id) AS sets_template) AS spawn_groups';
