...
$rows = $database->Query("DELETE FROM sentinel.dinos WHERE dino_id IN (SELECT DISTINCT dino_id FROM sentinel.dinos INNER JOIN sentinel.services ON (dinos.service_id = services.service_id) INNER JOIN public.user_subscriptions ON (services.user_id = user_subscriptions.user_id AND user_subscriptions.game_id = 'Sentinel') WHERE dinos.is_dead = TRUE AND dinos.last_update < CURRENT_TIMESTAMP - (user_subscriptions.metadata->>'retentionDays' || ' days')::INTERVAL) RETURNING dino_id;");
