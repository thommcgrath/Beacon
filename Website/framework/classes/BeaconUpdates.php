...
abstract class BeaconUpdates {
		...
		while ($rows->EOF() === false) {
			...
			if (count($best_downloads) > 0) {
				$updates[] = [
					...
					'supported_games' => json_decode($rows->Field('supported_games'), true)
				];
			}
	...
	public static function FindLatestInChannel(int $channel): ?array {
		$database = BeaconCommon::Database();
		$rows = $database->Query('SELECT update_id, build_number, build_display, stage, preview, notes, min_mac_version, min_win_version, EXTRACT(epoch FROM published) AS release_date, delta_version, ARRAY_TO_JSON(supported_games) AS supported_games FROM updates WHERE stage >= $1 ORDER BY build_number DESC LIMIT 1;', $channel);
		$updates = null;
	...
	public static function GetUpdateByUUID(string $uuid): ?array {
		$database = BeaconCommon::Database();
		$rows = $database->Query('SELECT update_id, build_number, build_display, stage, preview, notes, min_mac_version, min_win_version, EXTRACT(epoch FROM published) AS release_date, delta_version, ARRAY_TO_JSON(supported_games) AS supported_games FROM updates WHERE update_id = $1;', $uuid);
		$updates = null;
		if ($rows->RecordCount() === 0) {
			return $updates;
		}
	...
	public static function GetUpdateByBuildNumber(int $build_number): ?array {
		$database = BeaconCommon::Database();
		$rows = $database->Query('SELECT update_id, build_number, build_display, stage, preview, notes, min_mac_version, min_win_version, EXTRACT(epoch FROM published) AS release_date, delta_version, ARRAY_TO_JSON(supported_games) AS supported_games FROM updates WHERE build_number = $1;', $build_number);
		$updates = null;
		if ($rows->RecordCount() === 0) {
			return $updates;
		}
	...
	protected static function BuildArray(BeaconRecordSet $rows): array {
		...
		return [
			'update_id' => $update_id,
			'build_number' => filter_var($rows->Field('build_number'), FILTER_VALIDATE_INT),
			'build_display' => $rows->Field('build_display'),
			'stage' => filter_var($rows->Field('stage'), FILTER_VALIDATE_INT),
			'preview' => $rows->Field('preview'),
			'notes' => $rows->Field('notes'),
			'min_mac_version' => $rows->Field('min_mac_version'),
			'min_win_version' => $rows->Field('min_win_version'),
